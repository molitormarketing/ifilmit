...
const STATUS_CONFIG = {
  idea:            { label: "Idea",             emoji: "ðŸ’¡" },
  filming:         { label: "Filming",          emoji: "ðŸŽ¬" },
  uploaded_raw:    { label: "Needs Editing",    emoji: "ðŸ“¤" },
  uploaded_edited: { label: "Edited & Ready",   emoji: "âœ‚ï¸"  },
  published:       { label: "Published âœ“",      emoji: "ðŸš€" },
  pending:         { label: "Pending Approval", emoji: "â³" },
};
  ...
  clients: [
    {
      ...
      ideas:[
        {id:1,type:"Reel",hook:"POV: You just found out you can eat this for $3",caption:"Budget breakfast that actually slaps ðŸ‘ #budgetfitness #mealprep",tags:["fitness","budget","food"],status:"idea",notes:"",thread:[],uploadedFileName:null,uploadedAt:null,isUGC:false,deadline:null,brief:null},
        {id:2,type:"Reel",hook:"The $12 makeup dupe that replaced my $60 foundation",caption:"I tested it for 2 weeks ðŸ‘€ #makeupdupes",tags:["makeup","dupes"],status:"filming",notes:"",thread:[],uploadedFileName:null,uploadedAt:null,isUGC:false,deadline:null,brief:null},
        {id:3,type:"Post",hook:"My full-body workout with zero equipment (and a baby on my hip)",caption:"No gym needed ðŸ’ª #momlife #homeworkout",tags:["fitness","momlife"],status:"idea",notes:"",thread:[],uploadedFileName:null,uploadedAt:null,isUGC:true,deadline:"2024-06-15",brief:"Brand wants 60s reel, show product in first 3 seconds, include discount code JESS20"},
        {id:4,type:"Reel",hook:"I spent $50 at the grocery store. Here's everything I got.",caption:"Budget grocery haul ðŸ›’ #budgetfood",tags:["food","budget"],status:"uploaded_raw",notes:"Cut intro, add trending audio",thread:[{from:"agency",text:"Great footage! Can you trim the first 5 seconds?",time:"2h ago"},{from:"creator",text:"On it! Will re-upload by tonight",time:"1h ago"}],uploadedFileName:"grocery_haul.mp4",uploadedAt:"2024-06-01T10:00:00Z",isUGC:false,deadline:null,brief:null},
        {id:5,type:"Reel",hook:"Honest review: drugstore vs high-end blush",caption:"Swatches, wear test ðŸŒ¸ #blushreview",tags:["makeup","review"],status:"published",notes:"",thread:[],uploadedFileName:null,uploadedAt:null,isUGC:false,deadline:null,brief:null},
      ],
    },
    {
      ...
      ideas:[
        {id:10,type:"Reel",hook:"I decluttered my entire apartment in one weekend",caption:"Before & after ðŸ  #declutter",tags:["home","lifestyle"],status:"idea",notes:"",thread:[],uploadedFileName:null,uploadedAt:null,isUGC:false,deadline:null,brief:null},
        {id:11,type:"Post",hook:"My $200 apartment refresh from Target & TJ Maxx",caption:"Big impact, small budget ðŸ›’ #homedecor",tags:["home","budget"],status:"filming",notes:"",thread:[],uploadedFileName:null,uploadedAt:null,isUGC:true,deadline:"2024-06-20",brief:"Target collab â€” show 3 hero products, must tag @target"},
      ],
    },
  ],
};
...
function StatBar({ ideas, extra }) {
  ...
  const stats = [
    { label:"Total Ideas",    value: ideas.length,          emoji:"ðŸ’¡" },
    { label:"Filming Now",    value: counts.filming,        emoji:"ðŸŽ¬" },
    { label:"Needs Editing",  value: counts.uploaded_raw,   emoji:"ðŸ“¤" },
    { label:"Published",      value: counts.published,      emoji:"ðŸš€" },
    ...(extra||[]),
  ];
  return (
    ...
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#fff0f6 0%,#fce8f0 100%)", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div style={{ width:"100%", maxWidth:420, textAlign:"center" }}>
        <div style={{ marginBottom:32 }}>
          <div style={{ fontFamily:"Georgia,serif", fontSize:40, color:hp, letterSpacing:-1 }}>âœ¦ FilmIt âœ¦</div>
          ...
          {!role ? (
            <>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                {[
                  { r:"agency",  icon:"ðŸ¢", title:"Agency / Manager", sub:"View all clients, manage ideas & review uploads" },
                  { r:"creator", icon:"ðŸŽ¬", title:"Creator",           sub:"View your ideas, film content & upload files" },
                ].map(o => (
            ...
            <>
              <button onClick={()=>setRole(null)} style={{ background:"none", border:"none", color:"#999", fontSize:13, marginBottom:16, cursor:"pointer" }}>â† Back</button>
                ...
                {clients.map(c => (
                  <button key={c.id} onClick={()=>onLogin("creator",c.id)} style={{ padding:"14px 16px", borderRadius:14, border:`2px solid ${border}`, background:"#fff", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:12, transition:"all 0.15s" }}
                    ...
                    <div>
                      ...
                      <div style={{ fontSize:12, color:"#999" }}>{c.handle} Â· {c.niche}</div>
                    </div>
                  </button>
                ))}
              </div>
        ...
        <div style={{ marginTop:16, fontSize:12, color:"#bbb" }}>Demo mode â€” no password required</div>
      </div>
    </div>
  );
}
...
function AgencyDashboard({ clients, onSelectClient, onAddClient, onLogout }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#fdf6f8" }}>
      ...
      <StatBar ideas={allIdeas} extra={pending>0?[{label:"Pending Approval",value:pending,emoji:"â³"}]:[]} />
              ...
              <div key={client.id} onClick={()=>onSelectClient(client.id)} style={{ background:"#fff", borderRadius:18, border:`1.5px solid ${hasUrgent?hp+"60":border}`, padding:"20px 24px", cursor:"pointer", boxShadow:hasUrgent?"0 4px 20px rgba(255,0,102,0.10)":"0 2px 12px rgba(255,0,102,0.04)", transition:"all 0.2s", display:"flex", alignItems:"center", gap:18 }}
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
                    ...
                    {counts.pending>0 && <span style={{ background:"#f5f5f5", color:"#6b6b6b", borderRadius:20, padding:"2px 8px", fontSize:11, fontWeight:700 }}>â³ {counts.pending} pending</span>}
                    {counts.uploaded_raw>0 && <span style={{ background:hpBg, color:hp, borderRadius:20, padding:"2px 8px", fontSize:11, fontWeight:700 }}>âš¡ {counts.uploaded_raw} needs edit</span>}
                  </div>
                ...
                <div style={{ color:"#ccc", fontSize:20 }}>â†’</div>
              </div>
            );
          })}
        </div>
  ...
  return (
    <header style={{ background:"#fff", borderBottom:`1px solid ${border}`, padding:"14px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16 }}>
      <div style={{ display:"flex", alignItems:"center", gap:16 }}>
        {left}
        <div style={{ fontFamily:"Georgia,serif", fontSize:22, color:hp }}>âœ¦ FilmIt âœ¦</div>
...
function CSVUploadModal({ onImport, onClose }) {
  ...
  return (
    <Modal onClose={onClose} title="ðŸ“¥ Bulk Upload Ideas" wide>
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        <div style={{ background:hpBg, borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontWeight:700, fontSize:14, color:"#0a0a0a" }}>Step 1 â€” Download the template</div>
            <div style={{ fontSize:12, color:"#999", marginTop:2 }}>Fill it in with your ideas, then upload below</div>
          </div>
          <Btn variant="outline" small onClick={downloadTemplate}>â¬‡ Download Template</Btn>
        </div>
        <div>
          <label style={labelSt}>Step 2 â€” Upload your filled CSV</label>
        ...
        {preview.length>0 && (
          <div>
            <label style={labelSt}>Preview â€” {preview.length} ideas found</label>
            ...
            <div style={{ display:"flex", gap:10, marginTop:14 }}>
              ...
              <Btn onClick={()=>{ onImport(preview); onClose(); }} style={{ flex:2 }}>âœ“ Import {preview.length} Ideas</Btn>
            </div>
          </div>
        )}
            ...
            return (
              <div key={i} style={{ display:"flex", justifyContent:isMe?"flex-end":"flex-start" }}>
                <div style={{ maxWidth:"80%", background:isMe?hp:"#fff", color:isMe?"#fff":"#0a0a0a", borderRadius:isMe?"14px 14px 4px 14px":"14px 14px 14px 4px", padding:"8px 12px", fontSize:13, border:isMe?"none":`1px solid ${border}`, boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
                  <div style={{ fontSize:10, fontWeight:700, opacity:0.6, marginBottom:3 }}>{m.from==="agency"?"ðŸ¢ Agency":"ðŸŽ¬ Creator"} Â· {m.time}</div>
                  {m.text}
                </div>
              </div>
            );
      ...
      <div style={{ marginTop:14 }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ background:"#f0fdf4", border:"1.5px solid #86efac", borderRadius:12, overflow:"hidden" }}>
          ...
          <div style={{ padding:"10px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}>
            <div>
              <div style={{ fontSize:13, color:"#0a0a0a", fontWeight:600 }}>ðŸŽ¥ {idea.uploadedFileName}</div>
    ...
    <div style={{ background:"#fff", border:`2px solid ${isUGC?"#7c3aed60":idea.uploadedFileName?"#86efac":border}`, borderRadius:16, padding:"16px 20px", boxShadow:isUGC?"0 2px 16px rgba(124,58,237,0.08)":"0 2px 10px rgba(255,0,102,0.04)", transition:"all 0.2s" }}>
      ...
      {isUGC && (
        <div style={{ background:"linear-gradient(90deg,#7c3aed,#a855f7)", borderRadius:8, padding:"6px 12px", marginBottom:12, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <span style={{ color:"#fff", fontWeight:700, fontSize:12 }}>ðŸ’œ UGC / Brand Deal</span>
          {idea.deadline && <span style={{ color:"#e9d5ff", fontSize:11 }}>ðŸ“… Due {new Date(idea.deadline).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</span>}
        </div>
      )}
      ...
      <div onClick={()=>setExpanded(!expanded)} style={{ cursor:"pointer" }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10, marginBottom:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
            ...
            {idea.uploadedFileName && <span style={{ fontSize:11, color:"#16a34a", fontWeight:700 }}>ðŸŽ¥ attached</span>}
            {(idea.thread?.length||0)>0 && <span style={{ fontSize:11, color:hp, fontWeight:700 }}>ðŸ’¬ {idea.thread.length}</span>}
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ fontSize:12, color:"#ccc" }}>{expanded?"â–²":"â–¼"}</span>
            {isAgency && idea.status!=="pending" && <button onClick={e=>{e.stopPropagation();onDelete(idea.id);}} style={{ background:"none", border:"none", color:"#ddd", cursor:"pointer", fontSize:15 }}>âœ•</button>}
          </div>
        </div>
        ...
        <div style={{ marginTop:14, borderTop:`1px solid ${border}`, paddingTop:14 }}>
            ...
            <div style={{ marginBottom:14 }}>
              ...
              {idea.deadline && (
                <div style={{ marginTop:8, fontSize:12, color:"#7c3aed", fontWeight:600 }}>
                  ðŸ“… Deadline: {new Date(idea.deadline).toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})}
                </div>
              )}
            </div>
          )}
          ...
          {idea.status==="pending" && isAgency && (
            <div style={{ background:"#fefce8", border:"1px solid #fde047", borderRadius:12, padding:"12px 16px", marginBottom:14 }}>
              <div style={{ fontWeight:700, fontSize:13, color:"#854d0e", marginBottom:10 }}>â³ Creator submitted this idea â€” approve or reject?</div>
              <div style={{ display:"flex", gap:10 }}>
                <Btn onClick={e=>{e.stopPropagation();onStatusChange(idea.id,"idea");}} style={{ flex:1, background:"#16a34a", color:"#fff", border:"none" }}>âœ“ Approve</Btn>
                <Btn variant="danger" onClick={e=>{e.stopPropagation();onDelete(idea.id);}} style={{ flex:1 }}>âœ• Reject</Btn>
              </div>
            </div>
          )}
          ...
          {/* Status buttons â€” hide for pending if creator */}
...
function AddIdeaModal({ onAdd, onClose, isCreator }) {
  ...
  return (
    <Modal onClose={onClose} title={isCreator?"ðŸ’¡ Suggest an Idea":"Add New Idea"} wide>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", background:"#f5f0ff", borderRadius:10, border:"1px solid #c4b5fd", cursor:"pointer" }} onClick={()=>setForm(f=>({...f,isUGC:!f.isUGC}))}>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:13, color:"#4c1d95" }}>ðŸ’œ UGC / Brand Deal</div>
        ...
        <div style={{ display:"flex", gap:10, marginTop:4 }}>
          ...
          <Btn variant={isCreator?"outline":"primary"} onClick={()=>{
            ...
            onClose();
          }} style={{ flex:2 }}>{isCreator?"ðŸ“¤ Submit for Approval":"+ Add Idea"}</Btn>
        </div>
      </div>
    </Modal>
  );
...
function ClientBoard({ client, role, onBack, onUpdateClient, onLogout }) {
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#fdf6f8" }}>
      <Header
        left={isAgency&&<button onClick={onBack} style={{ background:"none", border:"none", color:hp, fontWeight:700, fontSize:14, cursor:"pointer" }}>â† All Clients</button>}
        title={`${client.name} Â· ${client.handle}`}
        right={
          <div style={{ display:"flex", gap:10, alignItems:"center" }}>
            {isAgency && <>
              <Btn variant="ghost" small onClick={()=>setShowCSV(true)}>ðŸ“¥ Bulk Upload</Btn>
              <Btn small onClick={()=>setShowAdd(true)}>+ Add Idea</Btn>
            </>}
            {!isAgency && <Btn variant="outline" small onClick={()=>setShowAdd(true)}>ðŸ’¡ Suggest Idea</Btn>}
            <span style={{ background:isAgency?hpBg:"#f0fdf4", color:isAgency?hp:"#16a34a", borderRadius:20, padding:"4px 12px", fontSize:12, fontWeight:700 }}>{isAgency?"ðŸ¢ Agency":"ðŸŽ¬ Creator"}</span>
      ...
      {/* Big stat bar */}
      <StatBar ideas={ideas} extra={counts.pending>0?[{label:"Pending Approval",value:counts.pending,emoji:"â³"}]:[]}/>
