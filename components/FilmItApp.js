...
function Logo() {
  return (
    <div style={{ textAlign: "center", marginBottom: 28 }}>
      <div style={{ fontFamily: "Georgia,serif", fontSize: 34, color: hp }}>✦ FilmIt ✦</div>
      <div style={{ fontSize: 13, color: "#999", marginTop: 4 }}>Content workflow for creators & agencies</div>
    </div>
  );
}
...
function Btn({ onClick, children, variant = "primary", disabled, style: x }) {
  ...
  const v = {
    ...
    ghost: { background: "transparent", color: "#999", border: `1px solid #e8e8e8` },
  };
...
function ErrorMsg({ msg }) {
  if (!msg) return null;
  return <div style={{ background: "#fff0f0", border: "1px solid #fca5a5", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#dc2626", marginBottom: 14 }}>{msg}</div>;
}

function SuccessMsg({ msg }) {
  if (!msg) return null;
  return <div style={{ background: "#f0fdf4", border: "1px solid #86efac", borderRadius: 8, padding: "10px 14px", fontSize: 13, color: "#16a34a", marginBottom: 14 }}>{msg}</div>;
}
...
function AgencySignup({ onBack, onSuccess }) {
  ...
  return (
    <div style={pageStyle}>
      <Logo />
      <div style={cardStyle}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 20, color: "#0a0a0a", marginBottom: 6 }}>Create Agency Account</div>
        <div style={{ fontSize: 13, color: "#999", marginBottom: 22 }}>You'll get a unique code to share with your creators</div>
        <ErrorMsg msg={error} />
    ...
    <div style={pageStyle}>
      ...
      <div style={cardStyle}>
        <div style={{ textAlign: "center", marginBottom: 24 }}>
          ...
          <div style={{ fontFamily: "Georgia,serif", fontSize: 20, color: "#0a0a0a", marginBottom: 6 }}>Agency Account Created!</div>
          <div style={{ fontSize: 13, color: "#999" }}>Share this code with your creators so they can join your workspace</div>
        </div>
        <div style={{ background: hpBg, border: `2px dashed ${hp}`, borderRadius: 14, padding: "20px", textAlign: "center", marginBottom: 20 }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: "#999", textTransform: "uppercase", letterSpacing: 0.8, marginBottom: 8 }}>Your Agency Code</div>
          <div style={{ fontFamily: "Georgia,serif", fontSize: 28, fontWeight: 700, color: hp, letterSpacing: 3 }}>{agencyCode}</div>
          <button onClick={copy} style={{ background: "none", border: "none", color: hp, fontSize: 13, fontWeight: 700, cursor: "pointer", marginTop: 8 }}>
            {copied ? "✓ Copied!" : "Copy code"}
          </button>
        </div>
        <div style={{ background: "#fefce8", border: "1px solid #fde047", borderRadius: 10, padding: "12px 14px", fontSize: 13, color: "#854d0e", marginBottom: 20 }}>
          <strong>Save this code!</strong> Your creators will need it to sign up. You can also find it later in your agency settings.
        </div>
        <div style={{ fontSize: 13, color: "#999", marginBottom: 16, textAlign: "center" }}>
...
function CreatorSignup({ onBack, onSuccess }) {
  ...
  return (
    <div style={pageStyle}>
      <Logo />
      <div style={cardStyle}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 20, color: "#0a0a0a", marginBottom: 6 }}>Creator Sign Up</div>
        <div style={{ fontSize: 13, color: "#999", marginBottom: 22 }}>You'll need the agency code your manager gave you</div>
        <ErrorMsg msg={error} />
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#999", marginTop: 4 }}>Ask your agency manager for this code</div>
          </div>
    ...
    <div style={pageStyle}>
      ...
      <div style={cardStyle}>
        <div style={{ textAlign: "center" }}>
          ...
          <div style={{ fontFamily: "Georgia,serif", fontSize: 20, color: "#0a0a0a", marginBottom: 8 }}>You're signed up!</div>
...
function LoginScreen({ onLogin }) {
  ...
  if (view === "choose-signup") return (
    <div style={pageStyle}>
      <Logo />
      <div style={cardStyle}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 20, color: "#0a0a0a", marginBottom: 20 }}>Create an Account</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <button onClick={() => setView("agency-signup")} style={{ padding: "16px 20px", borderRadius: 14, border: `2px solid ${border}`, background: hpBg, cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}
            onMouseEnter={e => e.currentTarget.style.borderColor = hp} onMouseLeave={e => e.currentTarget.style.borderColor = border}>
            <div style={{ fontWeight: 800, color: "#0a0a0a", fontSize: 15 }}>🏢 I'm an Agency / Manager</div>
            <div style={{ fontSize: 12, color: "#999", marginTop: 3 }}>Create a workspace and invite your creators</div>
          </button>
          <button onClick={() => setView("creator-signup")} style={{ padding: "16px 20px", borderRadius: 14, border: `2px solid ${border}`, background: "#fff", cursor: "pointer", textAlign: "left", transition: "all 0.15s" }}
            onMouseEnter={e => e.currentTarget.style.borderColor = hp} onMouseLeave={e => e.currentTarget.style.borderColor = border}>
            <div style={{ fontWeight: 800, color: "#0a0a0a", fontSize: 15 }}>🎬 I'm a Creator</div>
            <div style={{ fontSize: 12, color: "#999", marginTop: 3 }}>Join your agency's workspace with their code</div>
          </button>
  ...
  return (
    <div style={pageStyle}>
      <Logo />
      <div style={cardStyle}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 22, color: "#0a0a0a", marginBottom: 22 }}>Sign In</div>
        <ErrorMsg msg={error} />
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          <div><label style={labelSt}>Email</label><input type="email" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} placeholder="you@email.com" style={inputSt} /></div>
          <div><label style={labelSt}>Password</label><input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === "Enter" && handleLogin()} placeholder="Your password" style={inputSt} /></div>
          ...
          <div style={{ textAlign: "center", fontSize: 13, color: "#999" }}>
            ...
            <button onClick={() => setView("choose-signup")} style={{ background: "none", border: "none", color: hp, fontWeight: 700, cursor: "pointer", fontSize: 13 }}>Sign up</button>
          </div>
        </div>
      </div>
    </div>
...
export default function FilmItApp() {
  ...
  if (loading) return (
    <div style={{ minHeight: "100vh", background: "#fdf6f8", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 28, color: hp, marginBottom: 12 }}>✦ FilmIt ✦</div>
