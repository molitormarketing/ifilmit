...
function AgencyDashboard({ clients, onSelectClient, onAddClient, onLogout, userInfo }) {
  ...
  const ALL_NICHES_LIST = ["beauty","makeup","skincare","fashion","lifestyle","haircare","home"];
        ...
        {sidebarTab === "dashboard" && (
          <>
            ...
            <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>
              ...
              <div style={{ background:"#ffffff", borderRadius:18, border:"1px solid var(--border)", padding:"20px 24px" }}>
                ...
                <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                  {Object.entries(STATUS_CONFIG).map(([k,v]) => {
                    ...
                    return (
                      <div key={k} style={{ background:STATUS_COLORS[k].bg, borderRadius:12, padding:"12px 16px", flex:1, minWidth:100, textAlign:"center", border:`1px solid ${STATUS_COLORS[k].color}30` }}>
                        <div style={{ fontSize:20, marginBottom:4 }}>{v.emoji}</div>
                        <div style={{ fontSize:20, fontWeight:800, color:STATUS_COLORS[k].color }}>{count}</div>
