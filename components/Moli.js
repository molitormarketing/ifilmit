...
function CreatorCalendarTab({ client, role }) {
  ...
  const [eventForm, setEventForm] = useState({ title:"", time:"", note:"" });
  const [saving, setSaving] = useState(false);
  const isAgency = role === "agency";
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"28px 24px" }}>
          ...
          return (
            <div key={day} onClick={()=>handleDayClick(day)} style={{ minHeight:80, background: isToday?"rgba(255,0,102,0.08)":"#ffffff", border:`1.5px solid ${isToday?hp:"#f0d0dc"}`, borderRadius:10, padding:"8px 8px 6px", cursor:"pointer", transition:"all 0.15s" }}
              ...
              {dayEvents.slice(0,2).map(e => (
                <div key={e.id} style={{ background:hp, color:"#fff", borderRadius:4, padding:"2px 6px", fontSize:10, fontWeight:600, marginBottom:2, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
      ...
      {showEventForm && selectedDay && (
        <Modal onClose={()=>setShowEventForm(false)} title={`${monthName.split(" ")[0]} ${selectedDay}`}>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
            {eventsForDay(selectedDay).length > 0 && (
              <div style={{ marginBottom:8 }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#6b6b6b", textTransform:"uppercase", letterSpacing:0.8, marginBottom:8 }}>Events this day</div>
                {eventsForDay(selectedDay).map(e => (
                  <div key={e.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", background:hpBg, borderRadius:8, padding:"8px 12px", marginBottom:6 }}>
                    <div>
                      <div style={{ fontWeight:700, fontSize:13, color:"#0a0a0a" }}>{e.time?`${e.time} — `:""}{e.title}</div>
                      {e.note && <div style={{ fontSize:12, color:"#6b6b6b" }}>{e.note}</div>}
                    </div>
                    {isAgency && <button onClick={()=>handleDeleteEvent(e.id)} style={{ background:"none", border:"none", color:"#fca5a5", cursor:"pointer", fontSize:14 }}>✕</button>}
                  </div>
                ))}
              </div>
            )}
            {isAgency && (
              <>
                <div style={{ fontSize:11, fontWeight:700, color:"#6b6b6b", textTransform:"uppercase", letterSpacing:0.8 }}>Add Event</div>
                <input value={eventForm.title} onChange={e=>setEventForm(f=>({...f,title:e.target.value}))} placeholder="Event title" style={inputStCal}/>
                <input type="time" value={eventForm.time} onChange={e=>setEventForm(f=>({...f,time:e.target.value}))} style={inputStCal}/>
                <input value={eventForm.note} onChange={e=>setEventForm(f=>({...f,note:e.target.value}))} placeholder="Notes (optional)" style={inputStCal}/>
                <div style={{ display:"flex", gap:10 }}>
                  <Btn variant="ghost" onClick={()=>setShowEventForm(false)} style={{ flex:1 }}>Close</Btn>
                  <Btn onClick={handleAddEvent} disabled={saving||!eventForm.title.trim()} style={{ flex:2 }}>{saving?"Saving...":"Add Event"}</Btn>
                </div>
              </>
            )}
            {!isAgency && <Btn variant="ghost" onClick={()=>setShowEventForm(false)}>Close</Btn>}
          </div>
        </Modal>
      )}
    </div>
          ...
          return (
            <div key={m.id} style={{ display:"flex", justifyContent:isMe?"flex-end":"flex-start" }}>
              <div style={{ maxWidth:"70%", background:isMe?hp:"#ffffff", color:isMe?"#fff":"#0a0a0a", borderRadius:isMe?"16px 16px 4px 16px":"16px 16px 16px 4px", padding:"10px 14px", boxShadow:"0 1px 4px rgba(0,0,0,0.06)", border:isMe?"none":"1px solid #f0d0dc" }}>
                ...
                <div style={{ fontSize:10, marginTop:4, opacity:0.6, textAlign:isMe?"right":"left" }}>{m.from_role === "agency" ? "Agency" : client.name} · {time}</div>
              </div>
            </div>
          );
        })}
...
function ClientBoard({ client, role, onBack, onUpdateClient, onLogout }) {
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"#fdf6f8" }}>
      ...
      {/* Inbox tab */}
      {tab==="inbox" && (
        <InboxTab client={client} role={role}/>
      )}
