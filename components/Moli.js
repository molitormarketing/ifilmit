    ...
    <div style={{ display:"flex", minHeight:"100vh", background:"#fdf6f8" }}>
      ...
      <div style={{ flex:1, overflow:"auto" }}>
        <div style={{ background:"#ffffff", borderBottom:"1px solid var(--border)", padding:"16px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:18, fontWeight:700, color:"#0a0a0a", marginBottom:2 }}>Let's get to work 🔥</div>
...
const ALL_NICHES = ["beauty","makeup","skincare","fashion","lifestyle","haircare","home"];
const HEAT_COLORS = {
    ...
    <div style={{ maxWidth:960, margin:"0 auto", padding:"28px 24px" }}>
      ...
      <div style={{ display:"flex", gap:10, marginBottom:18, flexWrap:"wrap", alignItems:"center" }}>
        ...
        <div style={{ marginLeft:"auto", display:"flex", gap:8, alignItems:"center" }}>
          ...
          <Btn small onClick={()=>setShowSubmit(true)}>+ Submit a Brand</Btn>
        </div>
      </div>
      ...
      {/* Submit brand modal */}
      {showSubmit && (
        <Modal onClose={()=>setShowSubmit(false)} title="🎯 Submit a Brand">
          <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
            <div style={{ fontSize:13, color:"#6b6b6b" }}>Know a brand you'd love to work with? Submit it and your agency will review it.</div>
            ...
            <div><label style={labelSt}>Why this brand?</label><textarea value={submitForm.notes} onChange={e=>setSubmitForm(f=>({...f,notes:e.target.value}))} placeholder="Tell your agency why this is a good fit..." rows={3} style={inputStLight}/></div>
            <div style={{ display:"flex", gap:10 }}>
              <Btn variant="ghost" onClick={()=>setShowSubmit(false)} style={{ flex:1 }}>Cancel</Btn>
              <Btn onClick={handleSubmitBrand} disabled={submitting||!submitForm.brand_name.trim()} style={{ flex:2 }}>{submitting?"Submitting...":"Submit Brand"}</Btn>
            </div>
          </div>
        </Modal>
      )}
