...
function CreatorResourcesTab({ client, role }) {
  ...
  useEffect(() => {
    // Load general resources + creator-specific resources
    supabase.from("agency_resources").select("*")
      .or(`scope.eq.general,and(scope.eq.creator,client_id.eq.${client.id})`)
  ...
  const ResourceCard = ({ r }) => (
    <a href={r.url} target="_blank" rel="noopener noreferrer" style={{ textDecoration:"none" }}>
      <div style={{ background:"#ffffff", borderRadius:12, border:"1.5px solid #f0d0dc", padding:"14px 16px", display:"flex", alignItems:"flex-start", gap:12, cursor:"pointer", transition:"all 0.15s" }}
        onMouseEnter={e=>e.currentTarget.style.borderColor=hp}
...
function CreatorDashboardTab({ client, role }) {
  const isAgency = role === "agency";
  const today = new Date();
    ...
    <div style={{ maxWidth:960, margin:"0 auto", padding:"28px 24px" }}>
      ...
      {(website||mediaKit) && (
        <div style={{ display:"flex", gap:10, marginBottom:20, flexWrap:"wrap" }}>
          {website && <a href={website} target="_blank" rel="noopener noreferrer" style={{ textDecoration:"none" }}><div style={{ background:hpBg, border:`1.5px solid ${hp}`, borderRadius:10, padding:"8px 18px", fontSize:13, fontWeight:700, color:hp, cursor:"pointer" }}>🌐 Website →</div></a>}
          {mediaKit && <a href={mediaKit} target="_blank" rel="noopener noreferrer" style={{ textDecoration:"none" }}><div style={{ background:"#f5f0ff", border:"1.5px solid #7c3aed", borderRadius:10, padding:"8px 18px", fontSize:13, fontWeight:700, color:"#7c3aed", cursor:"pointer" }}>📎 Media Kit →</div></a>}
        </div>
      )}
      ...
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:20 }}>
        ...
        <MiniCard emoji="💡" label="Total Ideas" value={ideas.length} color="#0ea5e9" bg="#e0f2fe"/>
        <MiniCard emoji="🚀" label="Published" value={counts.published||0} color="#16a34a" bg="#f0fdf4"/>
      </div>
            ...
            {messages.map(m => (
              <div key={m.id} style={{ padding:"10px 14px", background:"#fdf6f8", borderRadius:10, border:"1px solid #f0d0dc" }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                  <span style={{ fontSize:11, fontWeight:700, color:m.from_role==="agency"?hp:"#7c3aed" }}>{m.from_role==="agency"?"🏢 Agency":"🎬 Creator"}</span>
...
function ClientBoard({ client, role, onBack, onUpdateClient, onLogout, userInfo }) {
  ...
  const tabs = [
    { key:"dash",       label:"Dashboard",       emoji:"🏠" },
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"#fdf6f8" }}>
      ...
      {/* Creator Dashboard tab */}
      {tab==="dash" && (
        <CreatorDashboardTab client={client} role={role}/>
      )}
