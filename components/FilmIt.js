...
const STATUS_COLORS = {
  idea:            { color: "#a78bfa", bg: "rgba(124,58,237,0.15)" },
  filming:         { color: "#ea580c", bg: "rgba(251,146,60,0.1)" },
  uploaded_raw:    { color: "#facc15", bg: "rgba(250,204,21,0.1)" },
  uploaded_edited: { color: "#4ade80", bg: "rgba(74,222,128,0.1)" },
  published:       { color: hp,        bg: hpBg      },
  pending:         { color: "rgba(255,255,255,0.65)", bg: "rgba(255,255,255,0.05)" },
};

const TAG_PALETTE = ["#ff0066","#7c3aed","#0ea5e9","#4ade80","#ea580c","#facc15"];
...
function Btn({ onClick, children, variant="primary", small, style:x, disabled }) {
  ...
  const v = { primary:{background:hp,color:"#1a1a2a"}, outline:{background:"transparent",color:hp,border:`1.5px solid ${hp}`}, ghost:{background:"transparent",color:"rgba(255,255,255,0.65)",border:"1px solid #e8e8e8"}, danger:{background:"transparent",color:"#f87171",border:"1px solid #fca5a5"}, purple:{background:"#7c3aed",color:"#1a1a2a"} };
...
const inputSt = { width:"100%", background:"#1a1a2a", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, padding:"9px 12px", color:"var(--text)", fontSize:13, outline:"none", resize:"vertical", boxSizing:"border-box" };
...
function StatBar({ ideas, extra }) {
  ...
  return (
    <div style={{ background:"#12121e", borderBottom:"1px solid var(--border)", padding:"16px 28px" }}>
...
function Modal({ onClose, title, children, wide }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.3)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"#1a1a2a", borderRadius:20, padding:28, width:"100%", maxWidth:wide?700:500, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 8px 48px rgba(255,0,102,0.15)" }}>
        <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"var(--text)", marginBottom:20 }}>{title}</div>
        {children}
      </div>
    </div>
  );
...
function AgencyDashboard({ clients, onSelectClient, onAddClient, onLogout, userInfo }) {
  ...
  const [sidebarTab, setSidebarTab] = useState("dashboard"); // month | alltime

  // Load unassigned creators
  useEffect(() => {
  ...
  const handleAssign = async (profileId) => {
    ...
    if (error) {
      ...
      alert("Could not assign creator: " + error.message);
      setAssigning(false);
      return;
    }
  ...
  const clientEarnings = clients.map(c => {
    ...
    return {
      id: c.id, name: c.name, handle: c.handle, avatar: c.avatar, color: c.color,
      monthTotal, monthAgency: monthTotal*(cut/100), monthCreator: monthTotal*(1-cut/100),
      allTotal, allAgency: allTotal*(cut/100), allCreator: allTotal*(1-cut/100),
      pendingDeals, cut,
    };
  });
  ...
  const MoneyCard = ({label,value,sub,color,bg}) => (
    <div style={{ background:bg||"rgba(255,0,102,0.12)", borderRadius:12, padding:"14px 16px", border:"1px solid var(--border)", flex:1, minWidth:120 }}>
      <div style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.65)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:6 }}>{label}</div>
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"#0d0d18" }}>
      ...
      <div style={{ flex:1, overflow:"auto" }}>
        <div style={{ background:"#12121e", borderBottom:"1px solid var(--border)", padding:"16px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:18, fontWeight:500, color:"#1a1a2a", marginBottom:2 }}>Good morning, {userInfo?.name?.split(" ")[0]||"there"}</div>
        ...
        {/* Sidebar tab: Earnings — agency-wide */}
        {sidebarTab === "earnings" && (
          <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>
            <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"var(--text)", marginBottom:20 }}>Agency Earnings</div>
            <div style={{ display:"grid", gap:16 }}>
              {clients.map(c => {
                const entries = c.earnings?.entries||[];
                const deals = (c.earnings?.brandDeals||[]).filter(d=>d.status==="paid");
                const total = entries.reduce((a,e)=>a+Number(e.amount||0),0) + deals.reduce((a,d)=>a+Number(d.amount||0),0);
                const agency = total * ((c.earnings?.agencyCut||20)/100);
                return total > 0 ? (
                  <div key={c.id} onClick={()=>onSelectClient(c.id)} style={{ background:"#1a1a2a", borderRadius:12, border:"1px solid var(--border)", padding:"16px 20px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                    <Avatar letter={c.avatar} color={c.color} size={44}/>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:600, color:"var(--text)", fontSize:14 }}>{c.name}</div>
                      <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)" }}>{c.handle}</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:18, fontWeight:700, color:"var(--text)" }}>{fmtMoney(total)}</div>
                      <div style={{ fontSize:12, color:hp }}>Your cut: {fmtMoney(agency)}</div>
                    </div>
                  </div>
                ) : null;
              })}
              {clients.every(c => (c.earnings?.entries||[]).length === 0) && (
                <div style={{ textAlign:"center", padding:"60px", color:"rgba(255,255,255,0.4)" }}>
                  <div style={{ fontSize:32, marginBottom:12 }}>💰</div>
                  <div>No earnings tracked yet — add entries in each creator's Earnings tab</div>
                </div>
              )}
            </div>
          </main>
        ...
        {/* Sidebar tab: Brands — coming soon */}
        {sidebarTab === "brands" && (
          <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>
            <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"var(--text)", marginBottom:20 }}>Brand Deals</div>
            <div style={{ display:"grid", gap:12 }}>
              {clients.flatMap(c => (c.earnings?.brandDeals||[]).map(d => ({ ...d, clientName: c.name, clientAvatar: c.avatar, clientColor: c.color, clientId: c.id }))).sort((a,b) => a.status==="pending" ? -1 : 1).map(deal => (
                <div key={deal.id} onClick={()=>onSelectClient(deal.clientId)} style={{ background:"#1a1a2a", borderRadius:12, border:`1px solid ${deal.status==="pending"?"rgba(250,204,21,0.2)":"rgba(255,255,255,0.1)"}`, padding:"14px 18px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                  <Avatar letter={deal.clientAvatar} color={deal.clientColor} size={36}/>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:600, color:"var(--text)", fontSize:14 }}>{deal.brand}</div>
                    <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)" }}>{deal.clientName} {deal.description ? `· ${deal.description}` : ""}</div>
                  </div>
                  <div style={{ textAlign:"right" }}>
                    <div style={{ fontSize:16, fontWeight:700, color:"var(--text)" }}>{fmtMoney(deal.amount)}</div>
                    <span style={{ fontSize:11, fontWeight:700, color:deal.status==="paid"?"#4ade80":"#facc15", background:deal.status==="paid"?"rgba(74,222,128,0.1)":"rgba(250,204,21,0.1)", borderRadius:20, padding:"2px 8px" }}>{deal.status==="paid"?"✓ Paid":"⏳ Pending"}</span>
                  </div>
                </div>
              ))}
              {clients.every(c => !(c.earnings?.brandDeals||[]).length) && (
                <div style={{ textAlign:"center", padding:"60px", color:"rgba(255,255,255,0.4)" }}>
                  <div style={{ fontSize:32, marginBottom:12 }}>🤝</div>
                  <div>No brand deals yet — add them in each creator's Earnings tab</div>
                </div>
              )}
            </div>
          </main>
        ...
        {/* Dashboard + Creators view */}
        {(sidebarTab === "dashboard" || sidebarTab === "creators") && (
        <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>

        {/* ── EARNINGS OVERVIEW ── */}
        <div style={{ background:"#1a1a2a", borderRadius:18, border:"1px solid var(--border)", padding:"20px 24px", marginBottom:24 }}>
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:18, flexWrap:"wrap", gap:10 }}>
            <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"var(--text)" }}>
              💰 Agency Earnings Overview
            </div>
            <div style={{ display:"flex", gap:8 }}>
              {["month","alltime"].map(v=>(
                <button key={v} onClick={()=>setEarningsView(v)} style={{
                  padding:"6px 14px", borderRadius:20, border:`1.5px solid ${earningsView===v?hp:border}`,
                  background:earningsView===v?hpBg:"transparent",
                  color:earningsView===v?hp:"#999", fontWeight:700, fontSize:12, cursor:"pointer",
                }}>{v==="month"?"This Month":"All Time"}</button>
              ))}
            </div>
          </div>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:20 }}>
            <MoneyCard
              label={earningsView==="month"?"Total Gross (Month)":"Total Gross (All Time)"}
              value={earningsView==="month"?totalMonthGross:totalAllGross}
              sub="Across all creators" color={hp} bg={hpBg}
            />
            <MoneyCard
              label={earningsView==="month"?"Agency Earns (Month)":"Agency Earns (All Time)"}
              value={earningsView==="month"?totalMonthAgency:totalAllAgency}
              sub="Your commission" color="#7c3aed" bg="rgba(124,58,237,0.12)"
            />
            <MoneyCard
              label="Pending Deals"
              value={totalPending}
              sub="Awaiting payment" color="#facc15" bg="rgba(250,204,21,0.08)"
            />
            <MoneyCard
              label="Published"
              value={publishedCount}
              sub="pieces of content" color="#4ade80" bg="rgba(74,222,128,0.1)"
            />
          </div>

          {/* Per creator breakdown */}
          {clientEarnings.some(c=>c.allTotal>0) && (
            <div>
              <div style={{ fontSize:11, fontWeight:700, color:"rgba(255,255,255,0.65)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:10 }}>Per Creator</div>
              <div style={{ display:"grid", gap:8 }}>
                {clientEarnings.map(c => {
                  const gross = earningsView==="month"?c.monthTotal:c.allTotal;
                  const agency = earningsView==="month"?c.monthAgency:c.allAgency;
                  if (gross===0 && c.pendingDeals===0) return null;
                  return (
                    <div key={c.id} onClick={()=>onSelectClient(c.id)} style={{ display:"flex", alignItems:"center", gap:14, padding:"12px 16px", background:"#0d0d18", borderRadius:12, cursor:"pointer", transition:"all 0.15s" }}
                      onMouseEnter={e=>e.currentTarget.style.background="rgba(255,0,102,0.08)"}
                      onMouseLeave={e=>e.currentTarget.style.background="rgba(255,255,255,0.06)"}>
                      <div style={{ width:36, height:36, borderRadius:"50%", background:c.color+"20", color:c.color, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:800, fontSize:14, flexShrink:0 }}>{c.avatar}</div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontWeight:700, fontSize:14, color:"var(--text)" }}>{c.name}</div>
                        <div style={{ fontSize:11, color:"rgba(255,255,255,0.65)" }}>{c.handle}</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontWeight:800, fontSize:15, color:"var(--text)" }}>{fmtMoney(gross)}</div>
                        <div style={{ fontSize:11, color:"#7c3aed" }}>Agency: {fmtMoney(agency)}</div>
                        {c.pendingDeals>0 && <div style={{ fontSize:11, color:"#facc15" }}>{fmtMoney(c.pendingDeals)} pending</div>}
                      </div>
                    </div>
                  );
                })}
          ...
          {clientEarnings.every(c=>c.allTotal===0) && (
            <div style={{ textAlign:"center", padding:"20px", color:"rgba(255,255,255,0.4)", fontSize:13 }}>
              No earnings tracked yet — add entries in each creator's Earnings tab
            </div>
          )}
        </div>

        {/* Unassigned creators banner */}
        {unassigned.length > 0 && (
          <div style={{ background:"#1a1a2a", borderRadius:16, border:`1.5px solid #fde047`, padding:"18px 20px", marginBottom:24 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
              <span style={{ fontSize:20 }}>⏳</span>
              <div>
                <div style={{ fontWeight:700, fontSize:15, color:"var(--text)" }}>{unassigned.length} creator{unassigned.length>1?"s":""} waiting to be assigned</div>
                <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)" }}>These creators signed up with your agency code but haven't been linked to a board yet</div>
              </div>
            </div>
            <div style={{ display:"grid", gap:10 }}>
              {unassigned.map(u => (
                <div key={u.id} style={{ background:"rgba(250,204,21,0.08)", borderRadius:12, padding:"12px 16px", border:"1px solid #fde047" }}>
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:12, flexWrap:"wrap" }}>
                    <div>
                      <div style={{ fontWeight:700, fontSize:14, color:"var(--text)" }}>{u.display_name || "Creator"}</div>
                      <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)" }}>{u.email}</div>
                    </div>
                    {assigningId === u.id ? (
        ...
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:18 }}>
          <div style={{ fontFamily:"Georgia,serif", fontSize:22, color:"var(--text)" }}>Your Clients</div>
        ...
        <div style={{ display:"grid", gap:14 }}>
          {clients.map(client => {
            ...
            return (
              <div key={client.id} onClick={()=>onSelectClient(client.id)} style={{ background:"#1a1a2a", borderRadius:18, border:`1.5px solid ${hasUrgent?hp:"rgba(255,255,255,0.12)"}`, padding:"20px 24px", cursor:"pointer", boxShadow:hasUrgent?"0 4px 20px rgba(255,0,102,0.10)":"0 2px 12px rgba(255,0,102,0.04)", transition:"all 0.2s", display:"flex", alignItems:"center", gap:18 }}
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
                    <div style={{ fontWeight:800, fontSize:16, color:"var(--text)" }}>{client.name}</div>
                    <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)" }}>{client.handle}</div>
                    {counts.pending>0 && <span style={{ background:"rgba(255,255,255,0.05)", color:"rgba(255,255,255,0.65)", borderRadius:20, padding:"2px 8px", fontSize:11, fontWeight:700 }}>⏳ {counts.pending} pending</span>}
                  ...
                  <div style={{ fontSize:13, color:"rgba(255,255,255,0.65)", marginBottom:10 }}>{client.niche}</div>
...
function Sidebar({ activeTab, onNavigate, onLogout, userInfo, clients }) {
  ...
  return (
    <div style={{ width:200, background:"#12121e", borderRight:"1px solid var(--border)", padding:"24px 12px", display:"flex", flexDirection:"column", flexShrink:0, minHeight:"100vh", position:"sticky", top:0, height:"100vh" }}>
      <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"#1a1a2a", padding:"0 8px", marginBottom:32, letterSpacing:-0.5 }}>
        ✦ <span style={{ color:hp }}>Moli</span>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
        {navItems.map(item => (
          <button key={item.key} onClick={()=>onNavigate(item.key)} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", borderRadius:8, fontSize:13, color:activeTab===item.key?hp:"rgba(255,255,255,0.6)", background:activeTab===item.key?"rgba(255,0,102,0.12)":"transparent", border:"none", cursor:"pointer", textAlign:"left", transition:"all 0.15s", width:"100%" }}
            onMouseEnter={e=>{ if(activeTab!==item.key) e.currentTarget.style.background="rgba(255,255,255,0.07)"; }}
      ...
      <div style={{ flex:1 }}/>
      <button onClick={onLogout} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", borderRadius:8, fontSize:13, color:"rgba(255,255,255,0.3)", background:"transparent", border:"none", cursor:"pointer", marginBottom:12, textAlign:"left", width:"100%" }}>
        <i className="ti ti-logout" style={{ fontSize:16 }} aria-hidden="true"/>
        Sign out
      </button>
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 10px 0", borderTop:"1px solid var(--border)" }}>
        <div style={{ width:30, height:30, borderRadius:"50%", background:"rgba(255,0,102,0.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, color:hp, fontWeight:700, flexShrink:0 }}>{initials}</div>
        <div>
          <div style={{ fontSize:12, color:"rgba(255,255,255,0.7)", lineHeight:1.2 }}>{userInfo?.name||"Agency"}</div>
          <div style={{ fontSize:10, color:"rgba(255,255,255,0.3)" }}>Agency</div>
        </div>
      </div>
    </div>
  );
...
function Header({ title, left, right }) {
  return (
    <header style={{ background:"#12121e", borderBottom:"1px solid var(--border)", padding:"14px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16 }}>
      <div style={{ display:"flex", alignItems:"center", gap:16 }}>
        {left}
        {title && <span style={{ fontSize:14, color:"rgba(255,255,255,0.65)", fontWeight:500 }}>{title}</span>}
      </div>
...
function AddClientModal({ onAdd, onClose, agencyCode }) {
  ...
  const colors = ["#ff0066","#7c3aed","#0ea5e9","#4ade80","#ea580c","#facc15"];
  ...
  if (added) return (
    <Modal onClose={onClose} title="Client Added!">
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ background:"rgba(74,222,128,0.1)", border:"1px solid #86efac", borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <div style={{ fontWeight:700, fontSize:15, color:"var(--text)" }}>{added.name} has been added!</div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)" }}>{added.handle}</div>
          </div>
        </div>

        <div>
          <div style={{ fontSize:13, fontWeight:700, color:"var(--text)", marginBottom:8 }}>Send this invite to {added.name}:</div>
          <div style={{ background:"#0d0d18", border:"1px solid var(--border)", borderRadius:10, padding:"12px 14px", fontSize:13, color:"rgba(255,255,255,0.65)", lineHeight:1.7, whiteSpace:"pre-wrap" }}>
            {inviteMessage}
          </div>
        </div>
        ...
        <div style={{ background:hpBg, border:`1px dashed ${hp}`, borderRadius:10, padding:"10px 14px", fontSize:13 }}>
          ...
          <span style={{ fontFamily:"monospace", letterSpacing:2, fontWeight:700, color:"var(--text)" }}>{agencyCode}</span>
        </div>
  ...
  return (
    <Modal onClose={onClose} title="Add New Client">
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ background:"rgba(250,204,21,0.08)", border:"1px solid #fde047", borderRadius:10, padding:"10px 14px", fontSize:12, color:"#facc15" }}>
    ...
    <Modal onClose={onClose} title="📥 Bulk Upload Ideas" wide>
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        <div style={{ background:hpBg, borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontWeight:700, fontSize:14, color:"var(--text)" }}>Step 1 — Download the template</div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)", marginTop:2 }}>Fill it in with your ideas, then upload below</div>
          </div>
        ...
        <div>
          ...
          <div onDragOver={e=>{e.preventDefault();}} onDrop={e=>{e.preventDefault();handleFile(e.dataTransfer.files[0]);}}
            ...
            style={{ border:`2px dashed ${border}`, borderRadius:12, padding:"24px", textAlign:"center", cursor:"pointer", background:"#0d0d18" }}>
            <div style={{ fontSize:28, marginBottom:6 }}>📄</div>
            <div style={{ fontSize:13, color:"rgba(255,255,255,0.65)" }}>Drop CSV here or tap to browse</div>
            ...
            <div style={{ maxHeight:200, overflowY:"auto", border:"1px solid var(--border)", borderRadius:10 }}>
              {preview.map((r,i)=>(
                <div key={i} style={{ padding:"10px 14px", borderBottom:"1px solid var(--border)", fontSize:13 }}>
                  <span style={{ background:r.type==="Reel"?"rgba(124,58,237,0.12)":"rgba(14,165,233,0.12)", color:r.type==="Reel"?"#7c3aed":"#0ea5e9", borderRadius:6, padding:"1px 8px", fontSize:11, fontWeight:700, marginRight:8 }}>{r.type||"Reel"}</span>
                  {r.isUGC==="true" && <span style={{ background:"rgba(124,58,237,0.12)", color:"#7c3aed", borderRadius:6, padding:"1px 8px", fontSize:11, fontWeight:700, marginRight:8, border:"1px solid #7c3aed40" }}>UGC</span>}
                  {r.hook}
                </div>
              ))}
            </div>
...
function Thread({ thread, onSend, role }) {
  ...
  return (
    <div style={{ marginTop:14 }} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{ background:"#0d0d18", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, overflow:"hidden" }}>
        <div style={{ maxHeight:180, overflowY:"auto", padding:"12px 14px", display:"flex", flexDirection:"column", gap:8 }}>
          {thread.length===0 && <div style={{ fontSize:12, color:"rgba(255,255,255,0.4)", textAlign:"center", padding:"12px 0" }}>No messages yet</div>}
          {thread.map((m,i)=>{
            const isMe = m.from===role;
            return (
              <div key={i} style={{ display:"flex", justifyContent:isMe?"flex-end":"flex-start" }}>
                <div style={{ maxWidth:"80%", background:isMe?hp:"#1a1a2a", color:isMe?"#1a1a2a":"var(--text)", borderRadius:isMe?"14px 14px 4px 14px":"14px 14px 14px 4px", padding:"8px 12px", fontSize:13, border:isMe?"none":`1px solid ${border}`, boxShadow:"0 1px 4px rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ borderTop:"1px solid var(--border)", display:"flex" }}>
          ...
          <button onClick={()=>{ if(msg.trim()){ onSend(msg.trim()); setMsg(""); }}} style={{ background:hp, color:"#1a1a2a", border:"none", padding:"10px 16px", fontWeight:700, fontSize:13, cursor:"pointer" }}>Send</button>
        </div>
      </div>
    </div>
  );
  ...
  if(idea.uploadedFileName) {
    ...
    return (
      <div style={{ marginTop:14 }} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{ background:"rgba(74,222,128,0.1)", border:"1px solid rgba(74,222,128,0.3)", borderRadius:12, overflow:"hidden" }}>
          ...
          <div style={{ padding:"10px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}>
            <div>
              <div style={{ fontSize:13, color:"var(--text)", fontWeight:600 }}>🎥 {idea.uploadedFileName}</div>
              <div style={{ fontSize:11, color:"rgba(255,255,255,0.65)", marginTop:2 }}>{sizeMB} MB</div>
            </div>
  ...
  return (
    <div style={{ marginTop:14 }} onClick={e=>e.stopPropagation()}>
      ...
      <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)}
        ...
        style={{ border:`2px dashed ${dragging?hp:border}`, borderRadius:12, padding:"20px", textAlign:"center", cursor:"pointer", background:dragging?hpBg:"rgba(255,255,255,0.06)", transition:"all 0.2s" }}>
        <div style={{ fontSize:24, marginBottom:6 }}>📁</div>
        <div style={{ fontSize:13, color:"rgba(255,255,255,0.65)", fontWeight:600 }}>Drop video or image here</div>
        <div style={{ fontSize:11, color:"rgba(255,255,255,0.4)", marginTop:3 }}>or tap to browse</div>
...
function IdeaCard({ idea, role, onStatusChange, onDelete, onUpload, onNoteChange, onThreadSend }) {
  ...
  return (
    <div style={{ background:"#1a1a2a", border:`2px solid ${isUGC?"#7c3aed60":idea.uploadedFileName?"rgba(134,239,172,0.3)":border}`, borderRadius:16, padding:"16px 20px", boxShadow:isUGC?"0 2px 16px rgba(124,58,237,0.08)":"0 2px 10px rgba(255,0,102,0.04)", transition:"all 0.2s" }}>
      {/* UGC banner */}
      {isUGC && (
        <div style={{ background:"linear-gradient(90deg,#7c3aed,#a855f7)", borderRadius:8, padding:"6px 12px", marginBottom:12, display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <span style={{ color:"#1a1a2a", fontWeight:700, fontSize:12 }}>💜 UGC / Brand Deal</span>
      ...
      <div onClick={()=>setExpanded(!expanded)} style={{ cursor:"pointer" }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10, marginBottom:8 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
            <span style={{ background:idea.type==="Reel"?"rgba(124,58,237,0.12)":"rgba(14,165,233,0.12)", color:idea.type==="Reel"?"#7c3aed":"#0ea5e9", borderRadius:6, padding:"2px 10px", fontSize:11, fontWeight:800, letterSpacing:0.5, textTransform:"uppercase" }}>{idea.type}</span>
            <Pill status={idea.status}/>
            {idea.uploadedFileName && <span style={{ fontSize:11, color:"#4ade80", fontWeight:700 }}>🎥 attached</span>}
            {(idea.thread?.length||0)>0 && <span style={{ fontSize:11, color:hp, fontWeight:700 }}>💬 {idea.thread.length}</span>}
          </div>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ fontSize:12, color:"rgba(255,255,255,0.4)" }}>{expanded?"▲":"▼"}</span>
        ...
        <div style={{ fontFamily:"Georgia,serif", fontSize:15, color:"var(--text)", lineHeight:1.5, marginBottom:8 }}>"{idea.hook}"</div>
      ...
      {expanded && (
        <div style={{ marginTop:14, borderTop:"1px solid var(--border)", paddingTop:14 }}>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <div style={{ fontSize:13, color:"rgba(255,255,255,0.65)", lineHeight:1.6, background:"#0d0d18", padding:"10px 12px", borderRadius:10 }}>{idea.caption}</div>
          </div>
          ...
          {isUGC && (
            <div style={{ marginBottom:14 }}>
              ...
              <div style={{ fontSize:13, color:"rgba(196,181,253,0.3)", lineHeight:1.6, background:"rgba(124,58,237,0.12)", padding:"10px 12px", borderRadius:10, border:"1px solid #c4b5fd" }}>
          ...
          {idea.status==="pending" && isAgency && (
            <div style={{ background:"rgba(250,204,21,0.08)", border:"1px solid #fde047", borderRadius:12, padding:"12px 16px", marginBottom:14 }}>
              <div style={{ fontWeight:700, fontSize:13, color:"#facc15", marginBottom:10 }}>⏳ Creator submitted this idea — approve or reject?</div>
              <div style={{ display:"flex", gap:10 }}>
                <Btn onClick={e=>{e.stopPropagation();onStatusChange(idea.id,"idea");}} style={{ flex:1, background:"#4ade80", color:"#1a1a2a", border:"none" }}>✓ Approve</Btn>
...
function AddIdeaModal({ onAdd, onClose, isCreator }) {
  ...
  return (
    <Modal onClose={onClose} title={isCreator?"💡 Suggest an Idea":"Add New Idea"} wide>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        {isCreator && <div style={{ background:"rgba(250,204,21,0.08)", borderRadius:10, padding:"10px 14px", fontSize:13, color:"#facc15" }}>Your idea will be sent to your agency for approval before it goes live on your board.</div>}
        <div>
          <label style={labelSt}>Content Type</label>
          <div style={{ display:"flex", gap:10 }}>
            {["Reel","Post"].map(t=><button key={t} onClick={()=>setForm(f=>({...f,type:t}))} style={{ flex:1, padding:10, borderRadius:10, border:`2px solid ${form.type===t?hp:border}`, background:form.type===t?hpBg:"#1a1a2a", color:form.type===t?hp:"#999", fontWeight:700, cursor:"pointer", fontSize:14 }}>{t}</button>)}
          </div>
        </div>
        ...
        {/* UGC toggle */}
        <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", background:"rgba(124,58,237,0.12)", borderRadius:10, border:"1px solid #c4b5fd", cursor:"pointer" }} onClick={()=>setForm(f=>({...f,isUGC:!f.isUGC}))}>
          <div style={{ width:40, height:22, borderRadius:11, background:form.isUGC?"#7c3aed":"#e5e7eb", transition:"all 0.2s", position:"relative" }}>
            <div style={{ width:18, height:18, borderRadius:"50%", background:"#1a1a2a", position:"absolute", top:2, left:form.isUGC?20:2, transition:"all 0.2s" }}/>
          </div>
          <div>
            <div style={{ fontWeight:700, fontSize:13, color:"rgba(196,181,253,0.3)" }}>💜 UGC / Brand Deal</div>
...
function EarningsTab({ client, role, onUpdateClient }) {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      ...
      {!isAgency && (
        <div style={{ background:"rgba(250,204,21,0.08)", border:"1px solid #fde047", borderRadius:12, padding:"12px 16px", marginBottom:20, fontSize:13, color:"#facc15" }}>
      ...
      {/* Agency settings bar */}
      {isAgency && (
        <div style={{ background:"#1a1a2a", borderRadius:14, padding:"14px 18px", border:"1px solid var(--border)", marginBottom:20, display:"flex", alignItems:"center", gap:20, flexWrap:"wrap" }}>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.65)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:4 }}>Agency Name (on invoices)</div>
            {agencyNameEdit ? (
          ...
          <div>
            <div style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.65)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:4 }}>Commission Rate</div>
            {agencyCutEdit ? (
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:16, flexWrap:"wrap", gap:10 }}>
        <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"var(--text)" }}>Earnings — {fmtMonth(month)}</div>
      ...
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:24 }}>
        ...
        <StatCard label="Creator Keeps" value={creatorEarns} sub={`${100-earnings.agencyCut}% of gross`} color="#4ade80" bg="rgba(74,222,128,0.1)"/>
        {isAgency && <StatCard label="Agency Earns" value={agencyEarns} sub={`${earnings.agencyCut}% commission`} color="#7c3aed" bg="rgba(124,58,237,0.12)"/>}
        <StatCard label="All-Time Gross" value={allTimeTotal} sub="All months" color="var(--text)" bg="#1a1a2a"/>
      </div>

      {/* Affiliate income */}
      <div style={{ background:"#1a1a2a", borderRadius:14, border:"1px solid var(--border)", padding:"18px 20px", marginBottom:20 }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
          <div style={{ fontWeight:700, fontSize:15, color:"var(--text)" }}>Affiliate Income</div>
        ...
        <div style={{ display:"grid", gap:10 }}>
          {[["Amazon Storefront","Amazon","#ea580c","📦"],[" LTK","LTK","#ff0066","🛍"],["Other Affiliate","Other","#7c3aed","🔗"]].map(([label,platform,color,icon])=>{
            ...
            return (
              <div key={platform} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 14px", background:"#0d0d18", borderRadius:10 }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <span style={{ fontSize:20 }}>{icon}</span>
                  <div>
                    <div style={{ fontWeight:700, fontSize:13, color:"var(--text)" }}>{label}</div>
                    <div style={{ fontSize:11, color:"rgba(255,255,255,0.65)" }}>{ents.length} {ents.length===1?"entry":"entries"}</div>
                  </div>
                </div>
                <div style={{ fontWeight:800, fontSize:16, color:val>0?color:"rgba(255,255,255,0.4)" }}>{fmtMoney(val)}</div>
              </div>
            );
          })}
        </div>
        ...
        {monthEntries.length>0 && (
          <div style={{ marginTop:14, borderTop:"1px solid var(--border)", paddingTop:14 }}>
            <div style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.65)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:8 }}>Line Items</div>
            <div style={{ display:"grid", gap:6 }}>
              {monthEntries.map(e=>(
                <div key={e.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", fontSize:13, padding:"6px 0", borderBottom:`1px solid #fdf6f8` }}>
                  <div style={{ color:"rgba(255,255,255,0.65)" }}>{e.platform}{e.note?` · ${e.note}`:""}</div>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                    <span style={{ fontWeight:700, color:"var(--text)" }}>{fmtMoney(e.amount)}</span>
        ...
        {monthEntries.length===0 && !isAgency && (
          <div style={{ textAlign:"center", padding:"20px", color:"rgba(255,255,255,0.4)", fontSize:13 }}>No affiliate entries added yet for {fmtMonth(month)}</div>
        )}
      </div>

      {/* Brand Deals */}
      <div style={{ background:"#1a1a2a", borderRadius:14, border:"1px solid var(--border)", padding:"18px 20px" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
          <div>
            <div style={{ fontWeight:700, fontSize:15, color:"var(--text)" }}>Brand Deals</div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)", marginTop:2 }}>{fmtMoney(pendingDeals)} pending · {fmtMoney(paidDeals)} paid all-time</div>
          </div>
          {isAgency && <Btn small variant="purple" onClick={()=>setShowAddDeal(true)}>+ Add Deal</Btn>}
        </div>
        {deals.length===0 && <div style={{ textAlign:"center", padding:"24px", color:"rgba(255,255,255,0.4)", fontSize:13 }}>No brand deals tracked yet</div>}
        <div style={{ display:"grid", gap:10 }}>
          {deals.map(deal=>(
            <div key={deal.id} style={{ background:deal.status==="paid"?"rgba(74,222,128,0.1)":"rgba(250,204,21,0.08)", borderRadius:12, padding:"14px 16px", border:`1px solid ${deal.status==="paid"?"rgba(134,239,172,0.3)":"rgba(253,224,71,0.2)"}` }}>
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10 }}>
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4, flexWrap:"wrap" }}>
                    <span style={{ fontWeight:700, fontSize:14, color:"var(--text)" }}>{deal.brand}</span>
                    <span style={{ fontSize:11, fontWeight:700, color:deal.status==="paid"?"#4ade80":"#facc15", background:deal.status==="paid"?"rgba(74,222,128,0.12)":"rgba(250,204,21,0.08)", borderRadius:20, padding:"1px 8px" }}>{deal.status==="paid"?"✓ Paid":"⏳ Pending"}</span>
                  </div>
                  {deal.description && <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)", marginBottom:4 }}>{deal.description}</div>}
                  <div style={{ display:"flex", gap:12, fontSize:11, color:"rgba(255,255,255,0.65)" }}>
                ...
                <div style={{ textAlign:"right", flexShrink:0 }}>
                  <div style={{ fontWeight:800, fontSize:18, color:"var(--text)" }}>{fmtMoney(deal.amount)}</div>
      ...
      {isAgency && monthTotal>0 && (
        <div style={{ marginTop:20, background:"linear-gradient(135deg,#fff0f6,#fce8f0)", borderRadius:14, padding:"16px 20px", border:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, flexWrap:"wrap" }}>
          <div>
            <div style={{ fontWeight:700, fontSize:14, color:"var(--text)" }}>Ready to invoice for {fmtMonth(month)}?</div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)", marginTop:2 }}>{client.name} owes <strong style={{ color:hp }}>{fmtMoney(agencyEarns)}</strong> ({earnings.agencyCut}% of {fmtMoney(monthTotal)})</div>
          </div>
      ...
      <div>
        ...
        <div style={{ display:"flex", gap:8 }}>
          {["Amazon","LTK","Other"].map(p=>(
            <button key={p} onClick={()=>setForm(f=>({...f,platform:p}))} style={{ flex:1, padding:"9px", borderRadius:10, border:`2px solid ${form.platform===p?hp:border}`, background:form.platform===p?hpBg:"#1a1a2a", color:form.platform===p?hp:"#999", fontWeight:700, cursor:"pointer", fontSize:13 }}>{p}</button>
          ))}
        </div>
      </div>
...
function ProfileTab({ client, role, onUpdateClient }) {
  ...
  const Section = ({ title, emoji, children }) => (
    <div style={{ background:"#1a1a2a", borderRadius:16, border:"1px solid var(--border)", padding:"20px 24px", marginBottom:16 }}>
      <div style={{ fontFamily:"Georgia,serif", fontSize:17, color:"var(--text)", marginBottom:16, display:"flex", alignItems:"center", gap:8 }}><span>{emoji}</span> {title}</div>
      {children}
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      <div style={{ display:"flex", justifyContent:"flex-end", marginBottom:12, height:20 }}>
        {saving && <span style={{ fontSize:12, color:"#facc15" }}>⟳ Saving...</span>}
      ...
      <Section title="Brand Comfort Level" emoji="✅">
        <div style={{ fontSize:13, color:"rgba(255,255,255,0.65)", marginBottom:14 }}>Toggle what this creator is comfortable promoting. Red = avoid.</div>
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
          {BRAND_PREFERENCES.map(pref => {
            const val = (profile.brandPrefs||{})[pref.key];
            return (
              <div key={pref.key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", background:val===false?"rgba(220,38,38,0.12)":val===true?"rgba(74,222,128,0.1)":"rgba(255,255,255,0.06)", borderRadius:10, border:`1px solid ${val===false?"rgba(252,165,165,0.3)":val===true?"rgba(134,239,172,0.3)":border}` }}>
                <span style={{ fontSize:13, fontWeight:600, color:"var(--text)" }}>{pref.emoji} {pref.label}</span>
                <div style={{ display:"flex", gap:6 }}>
                  <button onClick={()=>updateNested("brandPrefs", pref.key, true)} style={{ padding:"4px 12px", borderRadius:8, border:`1.5px solid ${val===true?"#4ade80":border}`, background:val===true?"rgba(74,222,128,0.1)":"#1a1a2a", color:val===true?"#4ade80":"#999", fontWeight:700, fontSize:12, cursor:"pointer" }}>Yes</button>
                  <button onClick={()=>updateNested("brandPrefs", pref.key, false)} style={{ padding:"4px 12px", borderRadius:8, border:`1.5px solid ${val===false?"#f87171":border}`, background:val===false?"rgba(220,38,38,0.12)":"#1a1a2a", color:val===false?"#f87171":"#999", fontWeight:700, fontSize:12, cursor:"pointer" }}>No</button>
                </div>
              </div>
            );
          })}
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      ...
      <div style={{ background:"linear-gradient(135deg,#fff0f6,#fce8f0)", border:"1px solid var(--border)", borderRadius:14, padding:"16px 20px", marginBottom:24, display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, flexWrap:"wrap" }}>
        <div>
          <div style={{ fontWeight:700, fontSize:15, color:"var(--text)" }}>Social Media Analytics</div>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:20, flexWrap:"wrap", gap:10 }}>
        <div style={{ fontFamily:"Georgia,serif", fontSize:18, color:"var(--text)" }}>Monthly Snapshot</div>
      ...
      <div style={{ display:"grid", gap:14 }}>
        {SOCIAL_PLATFORMS.map(platform => {
          ...
          return (
            <div key={platform.id} style={{ background:"#1a1a2a", borderRadius:16, border:`1.5px solid ${hasData?platform.color+"40":border}`, overflow:"hidden" }}>
              {/* Platform header */}
              <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div>
                    <div style={{ fontWeight:800, fontSize:15, color:"var(--text)" }}>{platform.name}</div>
                    <div style={{ fontSize:11, color:"rgba(255,255,255,0.65)" }}>{fmtMonth(selectedMonth)} metrics</div>
                  </div>
                </div>
                {isAgency && !isEditing && (
              ...
              <div style={{ padding:"0 20px 16px 20px" }}>
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(140px,1fr))", gap:10 }}>
                  {platform.metrics.map(metric => {
                    ...
                    return (
                      <div key={metric} style={{ background:hasData?"rgba(255,255,255,0.06)":"#fafafa", borderRadius:10, padding:"12px 14px", border:"1px solid var(--border)" }}>
                        <div style={{ fontSize:10, fontWeight:700, color:"rgba(255,255,255,0.65)", textTransform:"uppercase", letterSpacing:0.5, marginBottom:6 }}>{metric}</div>
                        {isEditing ? (
                          <input
                ...
                {!isEditing && hasData && (() => {
                  ...
                  if (!diff) return null;
                  return (
                    <div style={{ marginTop:10, fontSize:12, color:diff>0?"#4ade80":"#f87171", fontWeight:600 }}>
      ...
      {/* Coming soon note */}
      <div style={{ marginTop:24, background:"rgba(255,255,255,0.05)", borderRadius:14, padding:"16px 20px", textAlign:"center" }}>
        <div style={{ fontWeight:700, fontSize:14, color:"rgba(255,255,255,0.65)", marginBottom:4 }}>Auto-sync coming soon</div>
...
const APP_STATUS = {
  none:      { label:"Not Applied",  color:"rgba(255,255,255,0.4)",     bg:"#1a1a2a" },
  applied:   { label:"Applied",      color:"#facc15",  bg:"rgba(250,204,21,0.08)" },
  pending:   { label:"Awaiting Response", color:"#a78bfa", bg:"rgba(124,58,237,0.15)" },
  accepted:  { label:"Accepted!",    color:"#4ade80",  bg:"rgba(74,222,128,0.1)" },
  declined:  { label:"Declined",     color:"#f87171",  bg:"rgba(220,38,38,0.12)" },
...
function AddCustomPlatformForm({ onAdd, onClose }) {
  ...
  const colors = ["#ff0066","#7c3aed","#0ea5e9","#4ade80","#ea580c","#facc15","#ec4899","#f97316"];
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
      ...
      <div>
        <label style={labelSt}>Color</label>
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {["#ff0066","#7c3aed","#0ea5e9","#4ade80","#ea580c","#facc15","#ec4899","#f97316"].map(c=>(
...
function BrandPlatformsTab({ client, role, onUpdateClient }) {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      ...
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:24 }}>
        {[
          { label:"Platforms", value:PLATFORMS.length, emoji:"🔗", color:"var(--text)", bg:"#1a1a2a" },
          { label:"Active Applications", value:activeApps.length, emoji:"📋", color:"#a78bfa", bg:"rgba(124,58,237,0.15)" },
          { label:"Awaiting Response", value:pendingApps.length, emoji:"⏳", color:"#facc15", bg:"rgba(250,204,21,0.1)" },
          { label:"Accepted Deals", value:acceptedApps.length, emoji:"✓", color:"#4ade80", bg:"rgba(74,222,128,0.1)" },
        ].map(s=>(
      ...
      {activeApps.length > 0 && (
        <div style={{ background:"#1a1a2a", borderRadius:14, border:"1px solid var(--border)", padding:"18px 20px", marginBottom:20 }}>
          <div style={{ fontWeight:700, fontSize:15, color:"var(--text)", marginBottom:14 }}>Active Applications Pipeline</div>
          <div style={{ display:"grid", gap:8 }}>
            {activeApps.map(app => (
              <div key={app.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:APP_STATUS[app.status]?.bg||"#1a1a2a", borderRadius:10, border:`1px solid ${APP_STATUS[app.status]?.color}30` }}>
                <div style={{ width:10, height:10, borderRadius:"50%", background:app.platformColor, flexShrink:0 }}/>
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:700, fontSize:13, color:"var(--text)" }}>{app.brand} <span style={{ color:"rgba(255,255,255,0.65)", fontWeight:400 }}>via {app.platformName}</span></div>
                  {app.amount && <div style={{ fontSize:12, color:"#4ade80", fontWeight:600 }}>{fmtMoney(app.amount)}</div>}
                  {app.deadline && <div style={{ fontSize:11, color:"rgba(255,255,255,0.65)" }}>Due {new Date(app.deadline).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</div>}
                </div>
                <span style={{ fontSize:11, fontWeight:700, color:APP_STATUS[app.status]?.color, background:"#1a1a2a", borderRadius:20, padding:"3px 10px", border:`1px solid ${APP_STATUS[app.status]?.color}40` }}>
      ...
      {/* Platform list */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
        <div style={{ fontFamily:"Georgia,serif", fontSize:18, color:"var(--text)" }}>All Platforms</div>
      ...
      <div style={{ display:"grid", gap:12 }}>
        {allPlatforms.map(platform => {
          ...
          return (
            <div key={platform.id} style={{ background:"#1a1a2a", borderRadius:16, border:`1.5px solid ${hasActive?platform.color+"40":border}`, boxShadow:hasActive?`0 2px 12px ${platform.color}15`:"none", overflow:"hidden" }}>
              {/* Platform header */}
              <div onClick={()=>setExpandedPlatform(isExpanded?null:platform.id)} style={{ padding:"16px 20px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:2 }}>
                    <span style={{ fontWeight:800, fontSize:15, color:"var(--text)" }}>{platform.name}</span>
                    {accepted>0 && <span style={{ fontSize:11, fontWeight:700, color:"#4ade80", background:"rgba(74,222,128,0.1)", borderRadius:20, padding:"1px 8px" }}>{accepted} accepted</span>}
                    {pending>0 && <span style={{ fontSize:11, fontWeight:700, color:"#facc15", background:"rgba(250,204,21,0.08)", borderRadius:20, padding:"1px 8px" }}>{pending} pending</span>}
                  </div>
                  <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)" }}>{platform.desc}</div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
                  <a href={platform.url} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()} style={{ background:platform.color, color:"#1a1a2a", borderRadius:8, padding:"6px 14px", fontSize:12, fontWeight:700, textDecoration:"none", whiteSpace:"nowrap" }}>
                    Open Platform
                  </a>
                  <span style={{ color:"rgba(255,255,255,0.4)", fontSize:14 }}>{isExpanded?"▲":"▼"}</span>
                </div>
              </div>
              ...
              {isExpanded && (
                <div style={{ borderTop:"1px solid var(--border)", padding:"16px 20px" }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:"rgba(255,255,255,0.65)", textTransform:"uppercase", letterSpacing:0.6 }}>Applications ({apps.length})</div>
                    <Btn small onClick={()=>setShowAddApp(platform.id)}>+ Track Application</Btn>
                  </div>

                  {apps.length===0 && (
                    <div style={{ textAlign:"center", padding:"20px", color:"rgba(255,255,255,0.4)", fontSize:13, background:"#0d0d18", borderRadius:10 }}>
                  ...
                  <div style={{ display:"grid", gap:8, marginBottom:apps.length>0?14:0 }}>
                    {apps.map(app=>(
                      <div key={app.id} style={{ background:APP_STATUS[app.status]?.bg||"#1a1a2a", borderRadius:10, padding:"12px 14px", border:`1px solid ${APP_STATUS[app.status]?.color}30` }}>
                        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10 }}>
                          <div style={{ flex:1 }}>
                            <div style={{ fontWeight:700, fontSize:14, color:"var(--text)", marginBottom:3 }}>{app.brand}</div>
                            {app.description && <div style={{ fontSize:12, color:"rgba(255,255,255,0.65)", marginBottom:4 }}>{app.description}</div>}
                            {app.brief && <div style={{ fontSize:12, color:"rgba(196,181,253,0.3)", background:"rgba(124,58,237,0.12)", padding:"6px 10px", borderRadius:8, marginBottom:6, border:"1px solid #c4b5fd" }}>Brief: {app.brief}</div>}
                            <div style={{ display:"flex", gap:10, flexWrap:"wrap", fontSize:11, color:"rgba(255,255,255,0.65)" }}>
                              {app.amount && <span style={{ color:"#4ade80", fontWeight:700 }}>{fmtMoney(app.amount)}</span>}
                          ...
                          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6, flexShrink:0 }}>
                            <span style={{ fontSize:11, fontWeight:700, color:APP_STATUS[app.status]?.color, background:"#1a1a2a", borderRadius:20, padding:"3px 10px", border:`1px solid ${APP_STATUS[app.status]?.color}40` }}>{APP_STATUS[app.status]?.label}</span>
                            {isAgency && (
                              <select value={app.status} onChange={e=>{
                                updateAppStatus(platform.id, app.id, e.target.value);
                              }} style={{ fontSize:11, border:"1px solid var(--border)", borderRadius:6, padding:"2px 6px", color:"rgba(255,255,255,0.65)", background:"#1a1a2a" }}>
      ...
      <div>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {Object.entries(APP_STATUS).filter(([k])=>k!=="none").map(([k,v])=>(
            <button key={k} onClick={()=>setForm(f=>({...f,status:k}))} style={{ padding:"7px 14px", borderRadius:10, border:`2px solid ${form.status===k?v.color:border}`, background:form.status===k?v.bg:"#1a1a2a", color:form.status===k?v.color:"rgba(255,255,255,0.65)", fontWeight:700, cursor:"pointer", fontSize:12 }}>{v.label}</button>
          ))}
        </div>
      </div>
      <div style={{ display:"flex", gap:10, marginTop:4 }}>
...
function ClientBoard({ client, role, onBack, onUpdateClient, onLogout }) {
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"#0d0d18" }}>
      ...
      {/* Tab nav */}
      <div style={{ background:"#12121e", borderBottom:"1px solid var(--border)", padding:"0 24px" }}>
        <div style={{ maxWidth:960, margin:"0 auto", display:"flex", gap:0 }}>
          {tabs.map(t=>(
            <button key={t.key} onClick={()=>setTab(t.key)} style={{ padding:"14px 20px", border:"none", borderBottom:`2px solid ${tab===t.key?hp:"transparent"}`, background:"transparent", color:tab===t.key?hp:"rgba(255,255,255,0.4)", fontWeight:500, fontSize:13, cursor:"pointer", transition:"all 0.15s" }}>
              {t.emoji} {t.label}
            </button>
          ))}
        </div>
      ...
      {tab==="ideas" && (
        <>
          <div style={{ background:"#1a1a2a", borderBottom:"1px solid var(--border)", padding:"12px 24px" }}>
            <div style={{ maxWidth:900, margin:"0 auto" }}>
              <div style={{ display:"flex", gap:7, overflowX:"auto", paddingBottom:8, marginBottom:10 }}>
                {[["all","All",ideas.length],...Object.entries(STATUS_CONFIG).map(([k,v])=>[k,`${v.emoji} ${v.label}`,counts[k]||0])].map(([key,label,count])=>(
                  <button key={key} onClick={()=>setFilter(key)} style={{ background:filter===key?(key==="all"?"var(--text)":STATUS_COLORS[key]?.bg):"transparent", border:`1.5px solid ${filter===key?(key==="all"?"var(--text)":STATUS_COLORS[key]?.color+"60"):border}`, color:filter===key?(key==="all"?"#1a1a2a":STATUS_COLORS[key]?.color):"#999", borderRadius:20, padding:"5px 14px", cursor:"pointer", fontSize:12, fontWeight:700, whiteSpace:"nowrap" }}>
              ...
              <div style={{ display:"flex", gap:10 }}>
                ...
                {["all","Reel","Post"].map(t=>(
                  <button key={t} onClick={()=>setTypeFilter(t)} style={{ padding:"8px 14px", borderRadius:10, border:`1.5px solid ${typeFilter===t?hp:border}`, background:typeFilter===t?hpBg:"#1a1a2a", color:typeFilter===t?hp:"#999", cursor:"pointer", fontWeight:700, fontSize:13 }}>{t==="all"?"All":t}</button>
                ))}
              </div>
            </div>
          </div>
          <main style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
            {filtered.length===0?(
              <div style={{ textAlign:"center", padding:"60px 20px", color:"rgba(255,255,255,0.4)" }}>
                ...
                <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"rgba(255,255,255,0.65)" }}>No ideas here</div>
...
export default function FilmIt({ userInfo, onLogout }) {
  ...
  if (!loaded) return (
    <div style={{ minHeight: "100vh", background: "#0d0d18", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 14 }}>Loading your Moli workspace...</div>
      </div>
    </div>
  );
  ...
  if (role === "creator" && !activeClient) return (
    <div style={{ minHeight: "100vh", background: "#0d0d18", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ textAlign: "center", maxWidth: 400 }}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 28, color: "#ff0066", marginBottom: 12 }}>✦ Moli ✦</div>
        <div style={{ fontSize: 15, color: "rgba(255,255,255,0.65)", marginBottom: 20 }}>Your agency hasn't linked your account yet. Reach out to them to get set up!</div>
        <button onClick={handleLogout} style={{ background: "#ff0066", color: "#1a1a2a", border: "none", borderRadius: 10, padding: "10px 24px", fontWeight: 700, cursor: "pointer" }}>Sign Out</button>
      </div>
    </div>
  );

  return (
    <div style={{ minHeight: "100vh", background: "#0d0d18", display: "flex", alignItems: "center", justifyContent: "center", color: "rgba(255,255,255,0.4)" }}>
      Loading...
    </div>
  );
}
