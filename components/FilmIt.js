...
function Sidebar({ activeTab, onNavigate, onLogout, userInfo, clients }) {
  ...
  return (
    <div style={{ width:200, background:"#0d0d18", borderRight:"1px solid rgba(255,255,255,0.08)", padding:"24px 12px", display:"flex", flexDirection:"column", flexShrink:0, minHeight:"100vh", position:"sticky", top:0, height:"100vh" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
        {navItems.map(item => (
          <button key={item.key} onClick={()=>onNavigate(item.key)} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", borderRadius:8, fontSize:13, color:activeTab===item.key?hp:"rgba(255,255,255,0.55)", background:activeTab===item.key?"rgba(255,0,102,0.15)":"transparent", border:"none", cursor:"pointer", textAlign:"left", transition:"all 0.15s", width:"100%" }}
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 10px 0", borderTop:"1px solid rgba(255,255,255,0.08)" }}>
        <div style={{ width:30, height:30, borderRadius:"50%", background:"rgba(255,0,102,0.25)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, color:hp, fontWeight:700, flexShrink:0 }}>{initials}</div>
        <div>
          <div style={{ fontSize:12, color:"rgba(255,255,255,0.8)", lineHeight:1.2 }}>{userInfo?.name||userInfo?.agencyName||"Agency"}</div>
