...
function AgencyDashboard({ clients, onSelectClient, onAddClient, onLogout, userInfo }) {
  ...
  const handleAssign = async (profileId) => {
    ...
    setAssigning(true);
    await supabase.from("user_profiles").update({ client_id: assignClientId }).eq("id", profileId);
    setUnassigned(prev => prev.filter(u => u.id !== profileId));
                    ...
                    {assigningId === u.id ? (
                      <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap" }}>
                        <select
                          ...
                          style={{ ...inputSt, width:"auto", padding:"7px 12px", fontSize:13 }}
                        >
                          <option value="">Select a board...</option>
                          {clients.map(c => <option key={c.id} value={c.id}>{c.name} · {c.handle}</option>)}
                        </select>
                        <Btn small onClick={() => handleAssign(u.id)} disabled={!assignClientId || assigning}>
                          {assigning ? "Saving..." : "Assign"}
                        </Btn>
                        <Btn small variant="ghost" onClick={() => { setAssigningId(null); setAssignClientId(""); }}>Cancel</Btn>
                      </div>
                    ) : (
                      <Btn small onClick={() => { setAssigningId(u.id); setAssignClientId(""); }}>
                        Assign to Board →
                      </Btn>
                    )}
                  </div>
