...
const STATUS_COLORS = {
  ...
  pending:         { color: "var(--text-secondary)", bg: "#f5f5f5" },
};
...
function Btn({ onClick, children, variant="primary", small, style:x, disabled }) {
  ...
  const v = { primary:{background:hp,color:"#fff"}, outline:{background:"transparent",color:hp,border:`1.5px solid ${hp}`}, ghost:{background:"transparent",color:"var(--text-secondary)",border:"1px solid #e8e8e8"}, danger:{background:"transparent",color:"#dc2626",border:"1px solid #fca5a5"}, purple:{background:"#7c3aed",color:"#fff"} };
  return <button onClick={onClick} disabled={disabled} style={{...base,...v[variant]}}>{children}</button>;
}

const labelSt = { fontSize:10, fontWeight:800, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.8, display:"block", marginBottom:6 };
const inputSt = { width:"100%", background:"var(--bg-card)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, padding:"9px 12px", color:"var(--text)", fontSize:13, outline:"none", resize:"vertical", boxSizing:"border-box" };
...
function StatBar({ ideas, extra }) {
  ...
  return (
    <div style={{ background:"var(--bg-sidebar)", borderBottom:"1px solid var(--border)", padding:"16px 28px" }}>
      <div style={{ maxWidth:960, margin:"0 auto", display:"grid", gridTemplateColumns:`repeat(${stats.length}, 1fr)`, gap:12 }}>
        {stats.map(s => (
          <div key={s.label} style={{ background:"var(--bg-card)", borderRadius:12, padding:"14px 16px", border:"1px solid var(--border)", textAlign:"center" }}>
            ...
            <div style={{ fontSize:24, fontWeight:700, color:hp, lineHeight:1 }}>{s.value}</div>
...
function Modal({ onClose, title, children, wide }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.3)", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center", padding:20 }} onClick={onClose}>
      <div onClick={e=>e.stopPropagation()} style={{ background:"var(--bg-card)", borderRadius:20, padding:28, width:"100%", maxWidth:wide?700:500, maxHeight:"90vh", overflowY:"auto", boxShadow:"0 8px 48px rgba(255,0,102,0.15)" }}>
...
function AgencyDashboard({ clients, onSelectClient, onAddClient, onLogout, userInfo }) {
  ...
  const MoneyCard = ({label,value,sub,color,bg}) => (
    <div style={{ background:bg||"rgba(255,0,102,0.12)", borderRadius:12, padding:"14px 16px", border:"1px solid var(--border)", flex:1, minWidth:120 }}>
      <div style={{ fontSize:10, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:6 }}>{label}</div>
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"var(--bg)" }}>
      ...
      <div style={{ flex:1, overflow:"auto" }}>
        <div style={{ background:"var(--bg-sidebar)", borderBottom:"1px solid var(--border)", padding:"16px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:18, fontWeight:500, color:"#fff", marginBottom:2 }}>Good morning, {userInfo?.name?.split(" ")[0]||"there"}</div>
            <div style={{ fontSize:12, color:"rgba(255,255,255,0.35)" }}>{new Date().toLocaleDateString("en-US",{weekday:"long",month:"long",day:"numeric"})} {pending>0?`· ${pending} item${pending>1?"s":""} need your attention`:""}</div>
          </div>
          ...
          <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>
            ...
            <div style={{ display:"grid", gap:16 }}>
              {clients.map(c => {
                ...
                return total > 0 ? (
                  <div key={c.id} onClick={()=>onSelectClient(c.id)} style={{ background:"var(--bg-card)", borderRadius:12, border:"1px solid var(--border)", padding:"16px 20px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                    <Avatar letter={c.avatar} color={c.color} size={44}/>
                    <div style={{ flex:1 }}>
                      <div style={{ fontWeight:600, color:"var(--text)", fontSize:14 }}>{c.name}</div>
                      <div style={{ fontSize:12, color:"var(--text-secondary)" }}>{c.handle}</div>
                    </div>
          ...
          <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>
            ...
            <div style={{ display:"grid", gap:12 }}>
              {clients.flatMap(c => (c.earnings?.brandDeals||[]).map(d => ({ ...d, clientName: c.name, clientAvatar: c.avatar, clientColor: c.color, clientId: c.id }))).sort((a,b) => a.status==="pending" ? -1 : 1).map(deal => (
                <div key={deal.id} onClick={()=>onSelectClient(deal.clientId)} style={{ background:"var(--bg-card)", borderRadius:12, border:`1px solid ${deal.status==="pending"?"rgba(250,204,21,0.2)":"var(--border)"}`, padding:"14px 18px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ fontSize:12, color:"var(--text-secondary)" }}>{deal.clientName} {deal.description ? `· ${deal.description}` : ""}</div>
                  </div>
                  <div style={{ textAlign:"right" }}>
        ...
        {/* ── EARNINGS OVERVIEW ── */}
        <div style={{ background:"var(--bg-card)", borderRadius:18, border:"1px solid var(--border)", padding:"20px 24px", marginBottom:24 }}>
          ...
          <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:20 }}>
            ...
            <MoneyCard
              ...
              sub="Your commission" color="#7c3aed" bg="#f5f0ff"
            />
            <MoneyCard
              label="Pending Deals"
              value={totalPending}
              sub="Awaiting payment" color="#b45309" bg="#fefce8"
            />
            <MoneyCard
              label="Published"
              value={publishedCount}
              sub="pieces of content" color="#16a34a" bg="#f0fdf4"
            />
          </div>
          ...
          {clientEarnings.some(c=>c.allTotal>0) && (
            <div>
              <div style={{ fontSize:11, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:10 }}>Per Creator</div>
              <div style={{ display:"grid", gap:8 }}>
                {clientEarnings.map(c => {
                  ...
                  return (
                    <div key={c.id} onClick={()=>onSelectClient(c.id)} style={{ display:"flex", alignItems:"center", gap:14, padding:"12px 16px", background:"var(--bg)", borderRadius:12, cursor:"pointer", transition:"all 0.15s" }}
                      onMouseEnter={e=>e.currentTarget.style.background="#fce8f0"}
                      onMouseLeave={e=>e.currentTarget.style.background="#fdf6f8"}>
                      ...
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:11, color:"var(--text-secondary)" }}>{c.handle}</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
        ...
        {unassigned.length > 0 && (
          <div style={{ background:"var(--bg-card)", borderRadius:16, border:`1.5px solid #fde047`, padding:"18px 20px", marginBottom:24 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize:12, color:"var(--text-secondary)" }}>These creators signed up with your agency code but haven't been linked to a board yet</div>
              </div>
            </div>
        ...
        <div style={{ display:"grid", gap:14 }}>
          {clients.map(client => {
            ...
            return (
              <div key={client.id} onClick={()=>onSelectClient(client.id)} style={{ background:"var(--bg-card)", borderRadius:18, border:`1.5px solid ${hasUrgent?hp+"60":border}`, padding:"20px 24px", cursor:"pointer", boxShadow:hasUrgent?"0 4px 20px rgba(255,0,102,0.10)":"0 2px 12px rgba(255,0,102,0.04)", transition:"all 0.2s", display:"flex", alignItems:"center", gap:18 }}
                ...
                <div style={{ flex:1 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
                    ...
                    <div style={{ fontSize:12, color:"var(--text-secondary)" }}>{client.handle}</div>
                    {counts.pending>0 && <span style={{ background:"#f5f5f5", color:"var(--text-secondary)", borderRadius:20, padding:"2px 8px", fontSize:11, fontWeight:700 }}>⏳ {counts.pending} pending</span>}
                  ...
                  <div style={{ fontSize:13, color:"var(--text-secondary)", marginBottom:10 }}>{client.niche}</div>
...
function Sidebar({ activeTab, onNavigate, onLogout, userInfo, clients }) {
  ...
  return (
    <div style={{ width:200, background:"var(--bg-sidebar)", borderRight:"1px solid var(--border)", padding:"24px 12px", display:"flex", flexDirection:"column", flexShrink:0, minHeight:"100vh", position:"sticky", top:0, height:"100vh" }}>
      <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"#fff", padding:"0 8px", marginBottom:32, letterSpacing:-0.5 }}>
        ✦ <span style={{ color:hp }}>Moli</span>
      </div>
      <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
        {navItems.map(item => (
          <button key={item.key} onClick={()=>onNavigate(item.key)} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", borderRadius:8, fontSize:13, color:activeTab===item.key?hp:"rgba(255,255,255,0.45)", background:activeTab===item.key?"rgba(255,0,102,0.12)":"transparent", border:"none", cursor:"pointer", textAlign:"left", transition:"all 0.15s", width:"100%" }}
...
function Header({ title, left, right }) {
  return (
    <header style={{ background:"var(--bg-sidebar)", borderBottom:"1px solid var(--border)", padding:"14px 24px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16 }}>
      <div style={{ display:"flex", alignItems:"center", gap:16 }}>
        {left}
        {title && <span style={{ fontSize:14, color:"var(--text-secondary)", fontWeight:500 }}>{title}</span>}
      </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ background:"#f0fdf4", border:"1px solid #86efac", borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:"var(--text-secondary)" }}>{added.handle}</div>
          </div>
        </div>

        <div>
          <div style={{ fontSize:13, fontWeight:700, color:"var(--text)", marginBottom:8 }}>Send this invite to {added.name}:</div>
          <div style={{ background:"var(--bg)", border:"1px solid var(--border)", borderRadius:10, padding:"12px 14px", fontSize:13, color:"var(--text-secondary)", lineHeight:1.7, whiteSpace:"pre-wrap" }}>
            {inviteMessage}
          </div>
        </div>
  ...
  return (
    <Modal onClose={onClose} title="Add New Client">
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ background:"#fefce8", border:"1px solid #fde047", borderRadius:10, padding:"10px 14px", fontSize:12, color:"#854d0e" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        <div style={{ background:hpBg, borderRadius:12, padding:"14px 16px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            ...
            <div style={{ fontSize:12, color:"var(--text-secondary)", marginTop:2 }}>Fill it in with your ideas, then upload below</div>
          </div>
        ...
        <div>
          ...
          <div onDragOver={e=>{e.preventDefault();}} onDrop={e=>{e.preventDefault();handleFile(e.dataTransfer.files[0]);}}
            ...
            style={{ border:`2px dashed ${border}`, borderRadius:12, padding:"24px", textAlign:"center", cursor:"pointer", background:"var(--bg)" }}>
            <div style={{ fontSize:28, marginBottom:6 }}>📄</div>
            <div style={{ fontSize:13, color:"var(--text-secondary)" }}>Drop CSV here or tap to browse</div>
...
function Thread({ thread, onSend, role }) {
  ...
  return (
    <div style={{ marginTop:14 }} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{ background:"var(--bg)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, overflow:"hidden" }}>
        <div style={{ maxHeight:180, overflowY:"auto", padding:"12px 14px", display:"flex", flexDirection:"column", gap:8 }}>
          {thread.length===0 && <div style={{ fontSize:12, color:"var(--text-muted)", textAlign:"center", padding:"12px 0" }}>No messages yet</div>}
          {thread.map((m,i)=>{
        ...
        <div style={{ background:"#f0fdf4", border:"1px solid rgba(74,222,128,0.3)", borderRadius:12, overflow:"hidden" }}>
          ...
          <div style={{ padding:"10px 14px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:10 }}>
            <div>
              ...
              <div style={{ fontSize:11, color:"var(--text-secondary)", marginTop:2 }}>{sizeMB} MB</div>
            </div>
  ...
  return (
    <div style={{ marginTop:14 }} onClick={e=>e.stopPropagation()}>
      ...
      <div onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)}
        ...
        <div style={{ fontSize:13, color:"var(--text-secondary)", fontWeight:600 }}>Drop video or image here</div>
        <div style={{ fontSize:11, color:"var(--text-muted)", marginTop:3 }}>or tap to browse</div>
...
function IdeaCard({ idea, role, onStatusChange, onDelete, onUpload, onNoteChange, onThreadSend }) {
  ...
  return (
    <div style={{ background:"var(--bg-card)", border:`2px solid ${isUGC?"#7c3aed60":idea.uploadedFileName?"#86efac":border}`, borderRadius:16, padding:"16px 20px", boxShadow:isUGC?"0 2px 16px rgba(124,58,237,0.08)":"0 2px 10px rgba(255,0,102,0.04)", transition:"all 0.2s" }}>
      {/* UGC banner */}
      {isUGC && (
      ...
      {expanded && (
        <div style={{ marginTop:14, borderTop:"1px solid var(--border)", paddingTop:14 }}>
          ...
          <div style={{ marginBottom:14 }}>
            ...
            <div style={{ fontSize:13, color:"var(--text-secondary)", lineHeight:1.6, background:"var(--bg)", padding:"10px 12px", borderRadius:10 }}>{idea.caption}</div>
          </div>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:12, padding:"12px 14px", background:"#f5f0ff", borderRadius:10, border:"1px solid #c4b5fd", cursor:"pointer" }} onClick={()=>setForm(f=>({...f,isUGC:!f.isUGC}))}>
          <div style={{ width:40, height:22, borderRadius:11, background:form.isUGC?"#7c3aed":"#e5e7eb", transition:"all 0.2s", position:"relative" }}>
            <div style={{ width:18, height:18, borderRadius:"50%", background:"var(--bg-card)", position:"absolute", top:2, left:form.isUGC?20:2, transition:"all 0.2s" }}/>
          </div>
          <div>
...
function EarningsTab({ client, role, onUpdateClient }) {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      ...
      {/* Agency settings bar */}
      {isAgency && (
        <div style={{ background:"var(--bg-card)", borderRadius:14, padding:"14px 18px", border:"1px solid var(--border)", marginBottom:20, display:"flex", alignItems:"center", gap:20, flexWrap:"wrap" }}>
          <div style={{ flex:1 }}>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:4 }}>Agency Name (on invoices)</div>
            {agencyNameEdit ? (
          ...
          <div>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:4 }}>Commission Rate</div>
            {agencyCutEdit ? (
              <div style={{ display:"flex", gap:8, alignItems:"center" }}>
      ...
      <div style={{ background:"var(--bg-card)", borderRadius:14, border:"1px solid var(--border)", padding:"18px 20px", marginBottom:20 }}>
        ...
        <div style={{ display:"grid", gap:10 }}>
          {[["Amazon Storefront","Amazon","#ea580c","📦"],[" LTK","LTK","#ff0066","🛍"],["Other Affiliate","Other","#7c3aed","🔗"]].map(([label,platform,color,icon])=>{
            ...
            return (
              <div key={platform} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 14px", background:"var(--bg)", borderRadius:10 }}>
                <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                  <span style={{ fontSize:20 }}>{icon}</span>
                  <div>
                    <div style={{ fontWeight:700, fontSize:13, color:"var(--text)" }}>{label}</div>
                    <div style={{ fontSize:11, color:"var(--text-secondary)" }}>{ents.length} {ents.length===1?"entry":"entries"}</div>
                  </div>
                </div>
                <div style={{ fontWeight:800, fontSize:16, color:val>0?color:"var(--text-muted)" }}>{fmtMoney(val)}</div>
              </div>
            );
          })}
        </div>
        ...
        {monthEntries.length>0 && (
          <div style={{ marginTop:14, borderTop:"1px solid var(--border)", paddingTop:14 }}>
            <div style={{ fontSize:10, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.6, marginBottom:8 }}>Line Items</div>
            <div style={{ display:"grid", gap:6 }}>
              {monthEntries.map(e=>(
                <div key={e.id} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", fontSize:13, padding:"6px 0", borderBottom:`1px solid #fdf6f8` }}>
                  <div style={{ color:"var(--text-secondary)" }}>{e.platform}{e.note?` · ${e.note}`:""}</div>
                  <div style={{ display:"flex", alignItems:"center", gap:10 }}>
        ...
        {monthEntries.length===0 && !isAgency && (
          <div style={{ textAlign:"center", padding:"20px", color:"var(--text-muted)", fontSize:13 }}>No affiliate entries added yet for {fmtMonth(month)}</div>
        )}
      </div>
      ...
      <div style={{ background:"var(--bg-card)", borderRadius:14, border:"1px solid var(--border)", padding:"18px 20px" }}>
        <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:14 }}>
          <div>
            ...
            <div style={{ fontSize:12, color:"var(--text-secondary)", marginTop:2 }}>{fmtMoney(pendingDeals)} pending · {fmtMoney(paidDeals)} paid all-time</div>
          </div>
          {isAgency && <Btn small variant="purple" onClick={()=>setShowAddDeal(true)}>+ Add Deal</Btn>}
        </div>
        {deals.length===0 && <div style={{ textAlign:"center", padding:"24px", color:"var(--text-muted)", fontSize:13 }}>No brand deals tracked yet</div>}
        <div style={{ display:"grid", gap:10 }}>
          {deals.map(deal=>(
            <div key={deal.id} style={{ background:deal.status==="paid"?"#f0fdf4":"#fefce8", borderRadius:12, padding:"14px 16px", border:`1px solid ${deal.status==="paid"?"#86efac":"#fde047"}` }}>
              <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10 }}>
                <div style={{ flex:1 }}>
                  ...
                  {deal.description && <div style={{ fontSize:12, color:"var(--text-secondary)", marginBottom:4 }}>{deal.description}</div>}
      ...
      {isAgency && monthTotal>0 && (
        <div style={{ marginTop:20, background:"linear-gradient(135deg,#fff0f6,#fce8f0)", borderRadius:14, padding:"16px 20px", border:"1px solid var(--border)", display:"flex", alignItems:"center", justifyContent:"space-between", gap:16, flexWrap:"wrap" }}>
          <div>
            ...
            <div style={{ fontSize:12, color:"var(--text-secondary)", marginTop:2 }}>{client.name} owes <strong style={{ color:hp }}>{fmtMoney(agencyEarns)}</strong> ({earnings.agencyCut}% of {fmtMoney(monthTotal)})</div>
          </div>
...
function ProfileTab({ client, role, onUpdateClient }) {
  ...
  const Section = ({ title, emoji, children }) => (
    <div style={{ background:"var(--bg-card)", borderRadius:16, border:"1px solid var(--border)", padding:"20px 24px", marginBottom:16 }}>
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      ...
      <Section title="Brand Comfort Level" emoji="✅">
        <div style={{ fontSize:13, color:"var(--text-secondary)", marginBottom:14 }}>Toggle what this creator is comfortable promoting. Red = avoid.</div>
            ...
            return (
              <div key={pref.key} style={{ display:"flex", alignItems:"center", justifyContent:"space-between", padding:"10px 14px", background:val===false?"#fef2f2":val===true?"#f0fdf4":"#fdf6f8", borderRadius:10, border:`1px solid ${val===false?"#fca5a5":val===true?"#86efac":border}` }}>
                ...
                <div style={{ display:"flex", gap:6 }}>
                  <button onClick={()=>updateNested("brandPrefs", pref.key, true)} style={{ padding:"4px 12px", borderRadius:8, border:`1.5px solid ${val===true?"#16a34a":border}`, background:val===true?"#f0fdf4":"#fff", color:val===true?"#16a34a":"#999", fontWeight:700, fontSize:12, cursor:"pointer" }}>Yes</button>
                  <button onClick={()=>updateNested("brandPrefs", pref.key, false)} style={{ padding:"4px 12px", borderRadius:8, border:`1.5px solid ${val===false?"#dc2626":border}`, background:val===false?"#fef2f2":"#fff", color:val===false?"#dc2626":"#999", fontWeight:700, fontSize:12, cursor:"pointer" }}>No</button>
                </div>
              </div>
            );
          })}
    ...
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      ...
      <div style={{ display:"grid", gap:14 }}>
        {SOCIAL_PLATFORMS.map(platform => {
          ...
          return (
            <div key={platform.id} style={{ background:"var(--bg-card)", borderRadius:16, border:`1.5px solid ${hasData?platform.color+"40":border}`, overflow:"hidden" }}>
              {/* Platform header */}
              <div style={{ padding:"16px 20px", display:"flex", alignItems:"center", justifyContent:"space-between", gap:12 }}>
                <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize:11, color:"var(--text-secondary)" }}>{fmtMonth(selectedMonth)} metrics</div>
                  </div>
                </div>
                {isAgency && !isEditing && (
                  ...
                  {platform.metrics.map(metric => {
                    ...
                    return (
                      <div key={metric} style={{ background:hasData?"#fdf6f8":"#fafafa", borderRadius:10, padding:"12px 14px", border:"1px solid var(--border)" }}>
                        <div style={{ fontSize:10, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.5, marginBottom:6 }}>{metric}</div>
                        {isEditing ? (
                          <input
      ...
      <div style={{ marginTop:24, background:"#f5f5f5", borderRadius:14, padding:"16px 20px", textAlign:"center" }}>
        <div style={{ fontWeight:700, fontSize:14, color:"var(--text-secondary)", marginBottom:4 }}>Auto-sync coming soon</div>
...
const APP_STATUS = {
  none:      { label:"Not Applied",  color:"var(--text-muted)",     bg:"#f9f9f9" },
...
function BrandPlatformsTab({ client, role, onUpdateClient }) {
  ...
  return (
    <div style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
      ...
      <div style={{ display:"flex", gap:12, flexWrap:"wrap", marginBottom:24 }}>
        {[
          { label:"Platforms", value:PLATFORMS.length, emoji:"🔗", color:"var(--text)", bg:"#fff" },
          ...
          <div style={{ display:"grid", gap:8 }}>
            {activeApps.map(app => (
              <div key={app.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 14px", background:APP_STATUS[app.status]?.bg||"#f9f9f9", borderRadius:10, border:`1px solid ${APP_STATUS[app.status]?.color}30` }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:700, fontSize:13, color:"var(--text)" }}>{app.brand} <span style={{ color:"var(--text-secondary)", fontWeight:400 }}>via {app.platformName}</span></div>
                  {app.amount && <div style={{ fontSize:12, color:"#16a34a", fontWeight:600 }}>{fmtMoney(app.amount)}</div>}
                  {app.deadline && <div style={{ fontSize:11, color:"var(--text-secondary)" }}>Due {new Date(app.deadline).toLocaleDateString("en-US",{month:"short",day:"numeric"})}</div>}
                </div>
                <span style={{ fontSize:11, fontWeight:700, color:APP_STATUS[app.status]?.color, background:"var(--bg-card)", borderRadius:20, padding:"3px 10px", border:`1px solid ${APP_STATUS[app.status]?.color}40` }}>
      ...
      <div style={{ display:"grid", gap:12 }}>
        {allPlatforms.map(platform => {
          ...
          return (
            <div key={platform.id} style={{ background:"var(--bg-card)", borderRadius:16, border:`1.5px solid ${hasActive?platform.color+"40":border}`, boxShadow:hasActive?`0 2px 12px ${platform.color}15`:"none", overflow:"hidden" }}>
              {/* Platform header */}
              <div onClick={()=>setExpandedPlatform(isExpanded?null:platform.id)} style={{ padding:"16px 20px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                ...
                <div style={{ flex:1 }}>
                  ...
                  <div style={{ fontSize:12, color:"var(--text-secondary)" }}>{platform.desc}</div>
                </div>
                <div style={{ display:"flex", alignItems:"center", gap:8 }}>
              ...
              {isExpanded && (
                <div style={{ borderTop:"1px solid var(--border)", padding:"16px 20px" }}>
                  ...
                  <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:12 }}>
                    <div style={{ fontSize:12, fontWeight:700, color:"var(--text-secondary)", textTransform:"uppercase", letterSpacing:0.6 }}>Applications ({apps.length})</div>
                    <Btn small onClick={()=>setShowAddApp(platform.id)}>+ Track Application</Btn>
                  </div>

                  {apps.length===0 && (
                    <div style={{ textAlign:"center", padding:"20px", color:"var(--text-muted)", fontSize:13, background:"var(--bg)", borderRadius:10 }}>
                      ...
                      <div key={app.id} style={{ background:APP_STATUS[app.status]?.bg||"#f9f9f9", borderRadius:10, padding:"12px 14px", border:`1px solid ${APP_STATUS[app.status]?.color}30` }}>
                        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:10 }}>
                          <div style={{ flex:1 }}>
                            ...
                            {app.description && <div style={{ fontSize:12, color:"var(--text-secondary)", marginBottom:4 }}>{app.description}</div>}
                          ...
                          <div style={{ display:"flex", flexDirection:"column", alignItems:"flex-end", gap:6, flexShrink:0 }}>
                            <span style={{ fontSize:11, fontWeight:700, color:APP_STATUS[app.status]?.color, background:"var(--bg-card)", borderRadius:20, padding:"3px 10px", border:`1px solid ${APP_STATUS[app.status]?.color}40` }}>{APP_STATUS[app.status]?.label}</span>
                            {isAgency && (
                              <select value={app.status} onChange={e=>{
                                updateAppStatus(platform.id, app.id, e.target.value);
                              }} style={{ fontSize:11, border:"1px solid var(--border)", borderRadius:6, padding:"2px 6px", color:"var(--text-secondary)", background:"var(--bg-card)" }}>
      ...
      <div>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          {Object.entries(APP_STATUS).filter(([k])=>k!=="none").map(([k,v])=>(
            <button key={k} onClick={()=>setForm(f=>({...f,status:k}))} style={{ padding:"7px 14px", borderRadius:10, border:`2px solid ${form.status===k?v.color:border}`, background:form.status===k?v.bg:"#fff", color:form.status===k?v.color:"var(--text-secondary)", fontWeight:700, cursor:"pointer", fontSize:12 }}>{v.label}</button>
          ))}
        </div>
      </div>
      <div style={{ display:"flex", gap:10, marginTop:4 }}>
...
function ClientBoard({ client, role, onBack, onUpdateClient, onLogout }) {
  ...
  return (
    <div style={{ display:"flex", minHeight:"100vh", background:"var(--bg)" }}>
      ...
      {/* Tab nav */}
      <div style={{ background:"var(--bg-sidebar)", borderBottom:"1px solid var(--border)", padding:"0 24px" }}>
          ...
          <main style={{ maxWidth:900, margin:"0 auto", padding:"24px 20px" }}>
            {filtered.length===0?(
              <div style={{ textAlign:"center", padding:"60px 20px", color:"var(--text-muted)" }}>
                ...
                <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"var(--text-secondary)" }}>No ideas here</div>
  ...
  if (!loaded) return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <div style={{ color: "var(--text-muted)", fontSize: 14 }}>Loading your Moli workspace...</div>
      </div>
    </div>
  );
  ...
  if (role === "creator" && !activeClient) return (
    <div style={{ minHeight: "100vh", background: "var(--bg)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
      <div style={{ textAlign: "center", maxWidth: 400 }}>
        <div style={{ fontFamily: "Georgia,serif", fontSize: 28, color: "#ff0066", marginBottom: 12 }}>✦ Moli ✦</div>
        <div style={{ fontSize: 15, color: "var(--text-secondary)", marginBottom: 20 }}>Your agency hasn't linked your account yet. Reach out to them to get set up!</div>
