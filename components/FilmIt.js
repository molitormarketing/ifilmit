...
const inputSt = { width:"100%", background:"#ffffff", border:"1px solid rgba(255,255,255,0.1)", borderRadius:10, padding:"9px 12px", color:"#0a0a0a", fontSize:13, outline:"none", resize:"vertical", boxSizing:"border-box" };
    ...
    <div style={{ display:"flex", minHeight:"100vh", background:"#fdf6f8" }}>
      ...
      <div style={{ flex:1, overflow:"auto" }}>
        <div style={{ background:"#ffffff", borderBottom:"1px solid var(--border)", padding:"16px 28px", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
          <div>
            <div style={{ fontSize:18, fontWeight:500, color:"#ffffff", marginBottom:2 }}>Let's get to work 🔥</div>
        ...
        {sidebarTab === "earnings" && (
          <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>
            <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"#0a0a0a", marginBottom:20 }}>Agency Earnings</div>
            <div style={{ display:"grid", gap:16 }}>
              {clients.map(c => {
                ...
                return total > 0 ? (
                  <div key={c.id} onClick={()=>onSelectClient(c.id)} style={{ background:"#ffffff", borderRadius:12, border:"1px solid var(--border)", padding:"16px 20px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                    ...
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:18, fontWeight:700, color:"#0a0a0a" }}>{fmtMoney(total)}</div>
        ...
        {sidebarTab === "brands" && (
          <main style={{ maxWidth:960, margin:"0 auto", padding:"28px 28px" }}>
            <div style={{ fontFamily:"Georgia,serif", fontSize:20, color:"#0a0a0a", marginBottom:20 }}>Brand Deals</div>
            <div style={{ display:"grid", gap:12 }}>
              {clients.flatMap(c => (c.earnings?.brandDeals||[]).map(d => ({ ...d, clientName: c.name, clientAvatar: c.avatar, clientColor: c.color, clientId: c.id }))).sort((a,b) => a.status==="pending" ? -1 : 1).map(deal => (
                <div key={deal.id} onClick={()=>onSelectClient(deal.clientId)} style={{ background:"#ffffff", borderRadius:12, border:`1px solid ${deal.status==="pending"?"rgba(250,204,21,0.2)":"#f0d0dc"}`, padding:"14px 18px", cursor:"pointer", display:"flex", alignItems:"center", gap:14 }}>
                  <Avatar letter={deal.clientAvatar} color={deal.clientColor} size={36}/>
                  <div style={{ flex:1 }}>
                    <div style={{ fontWeight:600, color:"#0a0a0a", fontSize:14 }}>{deal.brand}</div>
                    <div style={{ fontSize:12, color:"#6b6b6b" }}>{deal.clientName} {deal.description ? `· ${deal.description}` : ""}</div>
                  </div>
                  <div style={{ textAlign:"right" }}>
                    <div style={{ fontSize:16, fontWeight:700, color:"#0a0a0a" }}>{fmtMoney(deal.amount)}</div>
                    <span style={{ fontSize:11, fontWeight:700, color:deal.status==="paid"?"#16a34a":"#b45309", background:deal.status==="paid"?"#f0fdf4":"rgba(250,204,21,0.1)", borderRadius:20, padding:"2px 8px" }}>{deal.status==="paid"?"✓ Paid":"⏳ Pending"}</span>
                  </div>
                </div>
              ))}
        ...
        <div style={{ background:"#ffffff", borderRadius:18, border:"1px solid var(--border)", padding:"20px 24px", marginBottom:24 }}>
          ...
          {clientEarnings.some(c=>c.allTotal>0) && (
            <div>
              <div style={{ fontSize:11, fontWeight:700, color:"#6b6b6b", textTransform:"uppercase", letterSpacing:0.6, marginBottom:10 }}>Per Creator</div>
              <div style={{ display:"grid", gap:8 }}>
                {clientEarnings.map(c => {
                  ...
                  return (
                    <div key={c.id} onClick={()=>onSelectClient(c.id)} style={{ display:"flex", alignItems:"center", gap:14, padding:"12px 16px", background:"#fdf6f8", borderRadius:12, cursor:"pointer", transition:"all 0.15s" }}
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontWeight:700, fontSize:14, color:"#0a0a0a" }}>{c.name}</div>
                        <div style={{ fontSize:11, color:"#6b6b6b" }}>{c.handle}</div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontWeight:800, fontSize:15, color:"#0a0a0a" }}>{fmtMoney(gross)}</div>
          ...
          {clientEarnings.every(c=>c.allTotal===0) && (
            <div style={{ textAlign:"center", padding:"20px", color:"#b0b0b0", fontSize:13 }}>
          ...
          <div style={{ background:"#ffffff", borderRadius:16, border:`1.5px solid #fde047`, padding:"18px 20px", marginBottom:24 }}>
            <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize:12, color:"#6b6b6b" }}>These creators signed up with your agency code but haven't been linked to a board yet</div>
              </div>
            </div>
...
function Sidebar({ activeTab, onNavigate, onLogout, userInfo, clients }) {
  ...
  return (
    <div style={{ width:200, background:"#ffffff", borderRight:"1px solid var(--border)", padding:"24px 12px", display:"flex", flexDirection:"column", flexShrink:0, minHeight:"100vh", position:"sticky", top:0, height:"100vh" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:2 }}>
        {navItems.map(item => (
          <button key={item.key} onClick={()=>onNavigate(item.key)} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", borderRadius:8, fontSize:13, color:activeTab===item.key?hp:"rgba(255,255,255,0.6)", background:activeTab===item.key?"rgba(255,0,102,0.15)":"transparent", border:"none", cursor:"pointer", textAlign:"left", transition:"all 0.15s", width:"100%" }}
            onMouseEnter={e=>{ if(activeTab!==item.key) e.currentTarget.style.background="#fdf0f4"; }}
      ...
      <div style={{ flex:1 }}/>
      <button onClick={onLogout} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", borderRadius:8, fontSize:13, color:"rgba(255,255,255,0.35)", background:"transparent", border:"none", cursor:"pointer", marginBottom:12, textAlign:"left", width:"100%" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 10px 0", borderTop:"1px solid var(--border)" }}>
        ...
        <div>
          <div style={{ fontSize:12, color:"rgba(255,255,255,0.7)", lineHeight:1.2 }}>{userInfo?.name||userInfo?.agencyName||"Agency"}</div>
          <div style={{ fontSize:10, color:"rgba(255,255,255,0.35)" }}>Agency</div>
        </div>
      </div>
    </div>
  );
...
function Thread({ thread, onSend, role }) {
  ...
  return (
    <div style={{ marginTop:14 }} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{ background:"#fdf6f8", border:"1px solid rgba(255,255,255,0.1)", borderRadius:12, overflow:"hidden" }}>
